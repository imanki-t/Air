...
useEffect(() => {
   ...
   if (!track || !dragger || !container || isPaginationEnabled || !showCustomScrollbar) {
       // Remove listeners if conditions are not met
       const handleMouseMove = () => {}; // No-op
       const handleMouseUp = () => {}; // No-op
       const handleTouchMove = () => {}; // No-op
   ...
   const updateDraggerPosition = () => {
       // Calculate dragger position based on container's scroll position
       ...
       if (containerScrollWidth <= containerClientWidth) {
            // Hide dragger if no scrollbar is needed
       ...
       const scrollPercentage = containerScrollLeft / (containerScrollWidth - containerClientWidth);
       const draggerPosition = scrollPercentage * (trackWidth - draggerWidth);
       dragger.style.transform = `translateX(${draggerPosition}px)`;
   };

   const handleMouseDown = (e) => {
       if (isPaginationEnabled || !showCustomScrollbar) return;
       isDragging = true;
       startX = e.clientX;
       draggerStartX = dragger.getBoundingClientRect().left;
       // Prevent default to avoid text selection while dragging
       e.preventDefault();
       dragger.classList.add('dragging'); // Add dragging style
   };

   const handleMouseMove = (e) => {
       if (!isDragging || isPaginationEnabled || !showCustomScrollbar) return;
       ...
       // Calculate new dragger position within track bounds
       let newDraggerLeft = draggerStartX + dx - track.getBoundingClientRect().left;
       newDraggerLeft = Math.max(0, Math.min(newDraggerLeft, trackWidth - draggerWidth));

       // Calculate corresponding scroll position for the container
       const scrollPercentage = newDraggerLeft / (trackWidth - draggerWidth);
       const newScrollLeft = scrollPercentage * (containerScrollWidth - containerClientWidth);

       container.scrollLeft = newScrollLeft;
       dragger.style.transform = `translateX(${newDraggerLeft}px)`; // Update dragger position directly
   };

   const handleMouseUp = () => {
       if (!isDragging) return;
       isDragging = false;
       dragger.classList.remove('dragging'); // Remove dragging style
   };

   const handleTouchStart = (e) => {
       if (isPaginationEnabled || !showCustomScrollbar || !e.touches[0]) return;
       isDragging = true;
   ...
   const handleTouchMove = (e) => {
        if (!isDragging || isPaginationEnabled || !showCustomScrollbar || !e.touches[0]) return;
       ...
       container.scrollLeft = newScrollLeft;
       dragger.style.transform = `translateX(${newDraggerLeft}px)`;
   };
   ...
   // Update dragger position when container scrolls (e.g., via mouse wheel)
   const handleContainerScroll = () => {
       if (!isDragging) { // Only update dragger if not currently dragging it
           updateDraggerPosition();
       }
   };

   // Initial dragger position setup and update on container resize
   const handleContainerResize = () => {
       updateDraggerPosition();
   };

   // Use ResizeObserver for more robust resize detection
   ...
   dragger.addEventListener('touchstart', handleTouchStart);
   ...
   return () => {
       ...
       dragger.removeEventListener('touchstart', handleTouchStart);
       document.removeEventListener('touchmove', handleTouchMove);
       document.removeEventListener('touchend', handleTouchEnd);
...
}, [isPaginationEnabled, showCustomScrollbar, paginatedFiles, sortedFiles, view]); // Re-run effect if these dependencies change
...
const sortedFiles = [...visible].sort((a, b) => {
  switch (sortOption) {
    ...
    default:    // Default sort (Latest First)
      return new Date(b.uploadDate) - new Date(b.uploadDate);
  }
});
// --- Pagination Logic ---
...
// Use sortedFiles for pagination regardless of custom scrollbar state
    ...
    <div className="flex flex-col md:flex-row gap-3 md:gap-4 mb-6 items-center justify-between flex-wrap">
      ...
      <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
          ...
          >
           ...
           {/* Custom Scrollbar Toggle (New Button - 3rd position) */}
           <button
               onClick={() => {
...
onClick={() => setShowSortOptions(prev => !prev)}
    ...
    <div className="min-h-[250px] relative"> {/* Added relative positioning for scrollbar */}
      {/* Files Display Area */}
      {isLoading ? (
        <div className={cn(
          'grid gap-4',
      ...
      ) : (isPaginationEnabled ? paginatedFiles : sortedFiles).length > 0 ? ( // Use sortedFiles when pagination is off
        <div
           ref={fileListContainerRef} // Ref for the container to scroll
           className={cn(
               'grid gap-4',
               view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1',
               // Apply horizontal scrolling and hide native scrollbar when custom scrollbar is active and pagination is off
               showCustomScrollbar && !isPaginationEnabled ? 'overflow-x-auto overflow-y-hidden whitespace-nowrap custom-scrollbar-hidden' : '',
               // Ensure items don't wrap when horizontal scrolling is active
               showCustomScrollbar && !isPaginationEnabled && view === 'grid' ? 'grid-flow-col auto-cols-[minmax(180px,_1fr)]' : '' // Adjust auto-cols as needed
           )}
        >
          {/* Render File Items */}
          {(isPaginationEnabled ? paginatedFiles : sortedFiles).map(file => ( // Use sortedFiles when pagination is off
            <FileItem
              key={file._id}
              file={file}
              darkMode={darkMode}
       ...
       {showCustomScrollbar && !isPaginationEnabled && sortedFiles.length > 0 && (
           <div
               ...
               className={cn(
                   'absolute bottom-0 left-0 right-0 h-2.5 rounded-full mx-auto max-w-full transition-opacity duration-300',
                   darkMode ? 'bg-gray-700' : 'bg-gray-300',
                   // Only show if there's actual content to scroll
                   fileListContainerRef.current?.scrollWidth > fileListContainerRef.current?.clientWidth ? 'opacity-100' : 'opacity-0 pointer-events-none'
               )}
               style={{ width: 'calc(100% - 2rem)' }} // Adjust width to account for padding
           >
               <div
                   ref={scrollbarDraggerRef}
                   className={cn(
                       'h-full rounded-full absolute top-0 left-0 cursor-pointer transition-colors duration-200',
                       darkMode ? 'bg-blue-500 hover:bg-blue-400' : 'bg-blue-600 hover:bg-blue-700',
                       'active:bg-blue-800 active:scale-y-110', // Add active state
                       'dragging:bg-blue-800 dragging:scale-y-110' // Style when dragging
                   )}
                    // Calculate initial dragger width based on content/container ratio
                    // This will be updated by the useEffect
                   style={{ width: `${Math.max(20, (fileListContainerRef.current?.clientWidth / fileListContainerRef.current?.scrollWidth) * 100) || 100}%` }}
               ></div>
           </div>
       )}
    </div>
...
<div className="mt-4 text-center">
  <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
    ...
    <style jsx>{`
      ...
      /* Hide native scrollbar for the file list container when custom scrollbar is active */
      ...
      .custom-scrollbar-hidden::-webkit-scrollbar {
       ...
       /* Style for dragger when being dragged */
       .dragging {
           /* Add any specific styles for when the dragger is active */
