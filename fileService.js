...
const getThumbnail = async (req, res) => {
  try {
    ...
    // Get the original image from Google Drive
    const driveResponse = await drive.files.get({
      fileId: driveId,
      alt: 'media'
    }, { responseType: 'stream' });
    ...
    // Resize to max 300x300 pixels while maintaining aspect ratio
    // Reduce quality to 70% for JPEG compression
    ...
    // If Sharp fails (unsupported format, corrupted image, etc.)
    // Fall back to serving original image with cache headers
    try {
...
module.exports = {
  ...
  deleteFile,
  downloadFile,
  getThumbnail, // New thumbnail function
  generateShareLink,
  accessSharedFile,
