...
const [isCustomScrollbarEnabled, setIsCustomScrollbarEnabled] = useState(false);
const [isDragging, setIsDragging] = useState(false);
const [startX, setStartX] = useState(0);
const [scrollLeft, setScrollLeft] = useState(0); // Scroll position of the content
const [draggerWidth, setDraggerWidth] = useState(0); // Width of the scrollbar dragger
const [draggerPosition, setDraggerPosition] = useState(0); // Position of the scrollbar dragger
...
const fileListContainerRef = useRef(null); // Ref for the scrollable file items container
const scrollbarTrackRef = useRef(null); // Ref for the custom scrollbar track
const scrollbarDraggerRef = useRef(null); // Ref for the custom scrollbar dragger
...
// Add isEditingPage to dependencies
...
// Use sortedFiles for total pages calculation
const startIndex = (currentPage - 1) * itemsPerPage;
const endIndex = startIndex + itemsPerPage;
// Apply pagination ONLY if enabled
const displayedFiles = isPaginationEnabled ? sortedFiles.slice(startIndex, endIndex) : sortedFiles;
...
displayedFiles : sortedFiles; // Use displayedFiles for current page selection
...
// Calculate dragger width and update on resize or content change
const calculateDraggerWidth = useCallback(() => {
   if (fileListContainerRef.current && scrollbarTrackRef.current) {
       const contentWidth = fileListContainerRef.current.scrollWidth;
       const containerWidth = fileListContainerRef.current.clientWidth;
       const trackWidth = scrollbarTrackRef.current.clientWidth;

       // Dragger width is proportional to the visible content area
       const width = (containerWidth / contentWidth) * trackWidth;
       setDraggerWidth(Math.max(width, 20)); // Ensure minimum dragger width (e.g., 20px)
   }
}, []);

// Sync dragger position with content scroll
const syncDraggerPosition = useCallback(() => {
   if (fileListContainerRef.current && scrollbarTrackRef.current && draggerWidth > 0) {
       const contentScrollLeft = fileListContainerRef.current.scrollLeft;
       const contentScrollWidth = fileListContainerRef.current.scrollWidth - fileListContainerRef.current.clientWidth;
       const trackWidth = scrollbarTrackRef.current.clientWidth;
       const draggerMovableWidth = trackWidth - draggerWidth;

       if (contentScrollWidth > 0) {
           const position = (contentScrollLeft / contentScrollWidth) * draggerMovableWidth;
           setDraggerPosition(position);
       } else {
            setDraggerPosition(0); // No scrolling needed if content fits
       }
   }
}, [draggerWidth]);

// Handle scroll event on the content
const handleContentScroll = useCallback(() => {
   if (!isDragging && isCustomScrollbarEnabled) { // Only sync dragger if not currently dragging it
       syncDraggerPosition();
   }
}, [isDragging, isCustomScrollbarEnabled, syncDraggerPosition]);


// Handle mouse/touch down on the dragger
const handleDraggerMouseDown = useCallback((e) => {
   if (!isCustomScrollbarEnabled) return;
   e.preventDefault();
   setIsDragging(true);
   setStartX(e.clientX || e.touches[0].clientX); // Use clientX for mouse, touches[0].clientX for touch
   setScrollLeft(fileListContainerRef.current.scrollLeft);
}, [isCustomScrollbarEnabled]);

// Handle mouse/touch move (global listener)
const handleMouseMove = useCallback((e) => {
   if (!isDragging || !fileListContainerRef.current || !scrollbarTrackRef.current || draggerWidth === 0) return;

   const clientX = e.clientX || e.touches[0].clientX;
   const dx = clientX - startX; // Distance moved horizontally

   const contentScrollWidth = fileListContainerRef.current.scrollWidth - fileListContainerRef.current.clientWidth;
   const trackWidth = scrollbarTrackRef.current.clientWidth;
   const draggerMovableWidth = trackWidth - draggerWidth;

   // Calculate scroll amount based on dragger movement ratio
   const scrollAmount = (dx / draggerMovableWidth) * contentScrollWidth;

   // Update content scroll position
   fileListContainerRef.current.scrollLeft = scrollLeft + scrollAmount;

   // Update dragger position (this will be synced by the content's scroll event handler, but we can also update directly for smoother dragging)
   // Let's rely on the content scroll event for syncing to avoid conflicting updates.
   // syncDraggerPosition(); // This would cause a loop if called here
}, [isDragging, startX, scrollLeft, draggerWidth]);

// Handle mouse/touch up (global listener)
...
const handleTrackClick = useCallback((e) => {
    // If pagination is enabled and the scrollbar area is clicked, disable pagination and enable custom scrollbar
    if (isPaginationEnabled) {
        setIsPaginationEnabled(false);
        setIsCustomScrollbarEnabled(true);
        // After enabling, the click should ideally move the dragger/scroll.
        // We can re-trigger the click logic or handle it here.
        // Let's re-trigger the logic after state updates.
        // A small timeout might be needed to ensure state updates are processed.
        setTimeout(() => handleTrackClick(e), 0);
        return; // Exit the first click handler
    }

    // If custom scrollbar is enabled and not dragging, jump to click position
    if (isCustomScrollbarEnabled && !isDragging && scrollbarTrackRef.current && fileListContainerRef.current) {
        const trackRect = scrollbarTrackRef.current.getBoundingClientRect();
        const clickX = e.clientX - trackRect.left; // Position relative to the track
        const trackWidth = trackRect.width;
        const draggerMovableWidth = trackWidth - draggerWidth;

        if (draggerMovableWidth <= 0) return; // Avoid division by zero

        // Calculate target dragger position
        const targetDraggerPosition = Math.max(0, Math.min(clickX - draggerWidth / 2, draggerMovableWidth));

        // Calculate target scroll position
        const contentScrollWidth = fileListContainerRef.current.scrollWidth - fileListContainerRef.current.clientWidth;
        const targetScrollLeft = (targetDraggerPosition / draggerMovableWidth) * contentScrollWidth;

        // Smooth scroll to the target position
        fileListContainerRef.current.scrollTo({
            left: targetScrollLeft,
            behavior: 'smooth'
        });
    }
}, [isPaginationEnabled, isCustomScrollbarEnabled, isDragging, draggerWidth]);


// Effect to add/remove global mouse/touch listeners for dragging
useEffect(() => {
   if (isDragging) {
...
// Effect to calculate initial dragger width and sync position
useEffect(() => {
   // Calculate width and sync position initially and whenever files or view changes
   calculateDraggerWidth();
   syncDraggerPosition();

   // Recalculate on window resize or when the file list container size might change
   ...
   if (fileListContainerRef.current) {
       resizeObserver.observe(fileListContainerRef.current);
   }
   ...
   return () => {
       // Cleanup observer and scroll listener
       if (fileListContainerRef.current) {
            // Check if observe was called before unobserving
            try {
               resizeObserver.unobserve(fileListContainerRef.current);
            } catch (e) {
                // Handle potential error if unobserve is called without observe
                console.warn("ResizeObserver unobserve failed, might not have been observed.", e);
            }
       }
        if (container) {
            container.removeEventListener('scroll', handleContentScroll);
        }
       resizeObserver.disconnect(); // Ensure observer is disconnected
   };
}, [files, view, calculateDraggerWidth, syncDraggerPosition, handleContentScroll]); // Re-run when files or view changes


// Effect to disable pagination when custom scrollbar is enabled
useEffect(() => {
    if (isCustomScrollbarEnabled && isPaginationEnabled) {
        setIsPaginationEnabled(false);
    }
}, [isCustomScrollbarEnabled, isPaginationEnabled]);

// Effect to ensure scrollbar is disabled if content is not scrollable horizontally
useEffect(() => {
   if (fileListContainerRef.current) {
       const isScrollable = fileListContainerRef.current.scrollWidth > fileListContainerRef.current.clientWidth;
       if (!isScrollable && isCustomScrollbarEnabled) {
           setIsCustomScrollbarEnabled(false);
       }
   }
}, [displayedFiles, isCustomScrollbarEnabled]); // Check when displayed files change
      ...
      <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
          ...
          <button
              ...
              className={cn(
                  ...
                  isPaginationEnabled && !isCustomScrollbarEnabled // Only highlight if pagination is ON and custom scrollbar is OFF
        ...
        >
         ...
         {/* Custom Scrollbar Toggle (Third Position) */}
          <button
              onClick={() => setIsCustomScrollbarEnabled(prev => !prev)}
              disabled={isPaginationEnabled} // Disable if pagination is ON
              className={cn(
                  'p-2 rounded-md transition-colors duration-200',
                  isCustomScrollbarEnabled && !isPaginationEnabled // Only highlight if custom scrollbar is ON and pagination is OFF
                      ? 'bg-blue-600 text-white hover:bg-blue-700'
                      : darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-600' : 'bg-gray-200 text-gray-600 hover:bg-gray-300 disabled:bg-gray-100 disabled:text-gray-400'
              )}
              ...
              title={isCustomScrollbarEnabled ? "Disable Custom Scrollbar" : "Enable Custom Scrollbar (Disables Pagination)"}
          >
               {/* Icon for Scrollbar - using a simple line with arrows or similar */}
               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 5l7 7-7 7M5 5l7 7-7 7" /> {/* Simple arrows */}
               </svg>
          </button>
...
onClick={() => setShowSortOptions(prev => !prev)}
      ...
      <div className={cn(
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
          ...
          <div className="w-full md:w-auto">
            <button
              ...
              className={cn(
               ...
               (isPaginationEnabled ? displayedFiles.every(file => selectedFiles.includes(file._id)) && displayedFiles.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0)
            ...
            >
              {(isPaginationEnabled ? displayedFiles.every(file => selectedFiles.includes(file._id)) && displayedFiles.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0) ? 'Deselect All' : 'Select All'}
            </button>
          </div>
          ...
          <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
             {selectedFiles.length > 0 && isPaginationEnabled ? `${selectedFiles.length} items selected on this page.` // Adjusted text for clarity with pagination
                : `${selectedFiles.length} of ${isPaginationEnabled ? displayedFiles.length : sortedFiles.length} selected.`
             }
          </div>
   ...
   {/* Custom Scrollbar Track (Visible when enabled and not loading) */}
   {isCustomScrollbarEnabled && !isLoading && displayedFiles.length > 0 && (
       <div
           ref={scrollbarTrackRef}
           className={cn(
               'relative w-full h-2 rounded-full mb-4 cursor-pointer',
               darkMode ? 'bg-gray-700' : 'bg-gray-300'
           )}
           onClick={handleTrackClick} // Handle click on the track
       >
           {/* Custom Scrollbar Dragger */}
           <div
               ref={scrollbarDraggerRef}
               className={cn(
                   'absolute top-0 h-full rounded-full',
                   isDragging ? (darkMode ? 'bg-blue-500' : 'bg-blue-700') : (darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-blue-500 hover:bg-blue-600'),
                   'transition-colors duration-150 ease-in-out'
               )}
               style={{
                   width: `${draggerWidth}px`,
                   left: `${draggerPosition}px`,
                   // Ensure dragger is visible only when scrollable
                   display: fileListContainerRef.current && fileListContainerRef.current.scrollWidth > fileListContainerRef.current.clientWidth ? 'block' : 'none',
               }}
               onMouseDown={handleDraggerMouseDown}
               onTouchStart={handleDraggerMouseDown}
           ></div>
       </div>
   )}


    {/* Files Display Area with min-height and custom scroll behavior */}
    <div
       ref={fileListContainerRef}
       className={cn(
          'min-h-[250px]',
          view === 'grid' ? 'grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid grid-cols-1 gap-4', // Ensure grid for list view too for gap
          isCustomScrollbarEnabled ? 'overflow-x-auto overflow-y-hidden custom-scrollbar-hidden' : 'overflow-auto', // Hide native horizontal scrollbar when custom is enabled
          // Add padding-bottom to prevent content from being hidden by potential native scrollbar space if not fully hidden
          'pb-1'
       )}
       // No need for onScroll here, added listener in useEffect
    >
      {isLoading ? (
        // Render Skeletons
        ...
        <div className={cn(
          'text-center py-16 rounded-lg border-2 border-dashed min-h-[200px] flex flex-col items-center justify-center', // Added flex for centering
    ...
    {/* Pagination Controls at the bottom (Hidden when custom scrollbar is enabled) */}
