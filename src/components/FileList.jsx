...
const FileItemSkeleton = ({ darkMode, view }) => (
 <div className={cn(
   'flex flex-col p-3 rounded-xl shadow-md border animate-pulse',
   view === 'grid' ? 'w-full h-[180px]' : 'w-full h-[80px] flex-row items-center', // Adjust height and layout for list view
   ...
   {view === 'grid' ? (
     ...
     <div className="flex items-center w-full">
       <div className={cn('w-10 h-10 rounded-md mr-3', darkMode ? 'bg-gray-700' : 'bg-gray-200')}></div>
...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
 ...
 // --- Scrollbar State ---
 const [isScrollbarEnabled, setIsScrollbarEnabled] = useState(false);
 ...
 const fileListContainerRef = useRef(null); // Ref for the file list display area
 const scrollbarThumbRef = useRef(null); // Ref for the custom scrollbar thumb
 ...
 const updateScrollbarThumb = useCallback(() => {
   ...
   const thumbWidth = Math.max(containerWidth / contentWidth * containerWidth, 20); // Min width of 20px
 ...
 useEffect(() => {
   ...
   // Update thumb position when the container is scrolled
 ...
 }, [updateScrollbarThumb, isScrollbarEnabled, isPaginationEnabled, sortedFiles.length, view]); // Dependencies include factors that affect scroll state/size
 ...
 const handleThumbMouseDown = (e) => {
   ...
   setScrollStartX(fileListContainerRef.current.scrollLeft);
 };
 ...
 const handleThumbMouseMove = (e) => {
   ...
   // Calculate the new scrollLeft based on thumb movement
   const scrollDelta = (deltaX / maxThumbLeft) * maxScrollLeft;
   container.scrollLeft = scrollStartX + scrollDelta;
 };
 ...
 useEffect(() => {
   if (isDragging) {
     ...
     document.addEventListener('touchmove', handleThumbMouseMove, { passive: false }); // Use passive: false for touch events
 ...
 }, [isDragging, handleThumbMouseMove]); // Add handleThumbMouseMove to dependencies
 ...
 return (
   <div className={cn(
       ...
       <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
         ...
         <button
           onClick={() => {
             ...
             // If enabling pagination, disable scrollbar
             if (!isPaginationEnabled) {
               setIsScrollbarEnabled(false);
             }
           }}
           className={cn(
         ...
         {/* Scrollbar Toggle (Third Icon) */}
         <button
           onClick={() => {
             setIsScrollbarEnabled(prev => !prev);
             // If enabling scrollbar, disable pagination
           ...
           aria-label={isScrollbarEnabled ? "Disable scrollbar" : "Enable scrollbar"}
           aria-pressed={isScrollbarEnabled}
           title={isScrollbarEnabled ? "Disable Scrollbar" : "Enable Scrollbar"}
         >
           {/* Icon for horizontal scrollbar */}
           <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
             <path strokeLinecap="round" strokeLinejoin="round" d="M9 4h6m-6 0v16M9 4H7a2 2 0 00-2 2v12a2 2 0 002 2h2m8-16h2a2 2 0 012 2v12a2 2 0 01-2 2h-2M9 4h7v16H9V4z" />
           </svg>
         </button>
     ...
     {/* File Display Area and Scrollbar Container */}
     <div className={cn(
       'relative', // Needed for absolute positioning of the scrollbar
       (isScrollbarEnabled && !isPaginationEnabled) ? 'pb-4' : '' // Add padding at the bottom when scrollbar is visible
     )}>
       {/* Files Display Area */}
       <div
         ...
         className={cn(
           'min-h-[250px]',
           view === 'grid' ? 'grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid grid-cols-1 gap-4',
           // Conditional overflow based on scrollbar and pagination state
           (isScrollbarEnabled && !isPaginationEnabled) ? 'overflow-x-auto overflow-y-hidden pb-2' : 'overflow-y-auto overflow-x-hidden', // Add padding-bottom to prevent content being hidden by scrollbar
           // Hide native scrollbar when custom is enabled
           (isScrollbarEnabled && !isPaginationEnabled) ? 'scrollbar-hide' : ''
         )}
         // Add a minimum width to the grid/list when scrollbar is enabled to ensure horizontal scrolling
         style={{ minWidth: (isScrollbarEnabled && !isPaginationEnabled) ? (view === 'grid' ? '800px' : '600px') : 'auto' }}
       >
         {isLoading ? (
           // Render Skeletons
           Array(view === 'grid' ? 10 : 5).fill().map((_, i) => (
             <FileItemSkeleton key={`skel-${i}`} darkMode={darkMode} view={view} />
           ))
           ...
           <div className={cn(
             ...
             'col-span-full flex flex-col items-center justify-center', // Center content in the available space
       ...
       {isScrollbarEnabled && !isPaginationEnabled && sortedFiles.length > 0 && (
         <div
           className={cn(
             'absolute bottom-0 left-0 right-0 h-2 rounded-full',
             darkMode ? 'bg-gray-700' : 'bg-gray-300'
           )}
           // Add a click handler to scroll when clicking the track (optional)
           onClick={(e) => {
         ...
         >
           <div
             ...
             className={cn(
               'h-full rounded-full cursor-pointer transition-colors duration-200',
             ...
             style={{ width: '0px' }} // Initial width, will be set by updateScrollbarThumb
           />
         </div>
       )}
     </div>
     ...
     <style jsx>{`
       ...
       /* Custom Scrollbar Styles (Hide native scrollbar) */
       .scrollbar-hide {
