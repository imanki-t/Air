...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
 ...
 const [isCustomScrollEnabled, setIsCustomScrollEnabled] = useState(false); // Custom scrollbar toggle, default OFF
 ...
 const fileListContainerRef = useRef(null); // Ref for the scrollable file list container
 ...
 useEffect(() => {
   const handleClickOutside = e => {
     ...
     // Close batch share modal only if not loading
 ...
 // Use paginated files only if pagination is enabled AND custom scroll is NOT enabled
 const displayFiles = isPaginationEnabled && !isCustomScrollEnabled ?
   sortedFiles.slice(startIndex, endIndex) :
   sortedFiles; // Show all files if pagination is off or custom scroll is on
 ...
 const handlePageClick = () => {
   // Only allow editing if pagination is enabled, custom scroll is NOT enabled, and there are pages
   if (isPaginationEnabled && !isCustomScrollEnabled && totalPages > 0) {
 ...
 const updateScrollbar = useCallback(() => {
   ...
   if (container && isCustomScrollEnabled) {
     const { scrollTop, scrollHeight, clientHeight } = container;
     const trackHeight = customScrollbarRef.current?.clientHeight || 0;

     // Calculate handle height: ratio of visible height to total height
     const handleRatio = clientHeight / scrollHeight;
     const newHandleHeight = Math.max(20, handleRatio * trackHeight); // Minimum handle height of 20px

     // Calculate handle position: ratio of scrollTop to scrollable height
     const scrollableHeight = scrollHeight - clientHeight;
     const handleScrollableHeight = trackHeight - newHandleHeight;
     const newHandlePosition = scrollableHeight > 0 ? (scrollTop / scrollableHeight) * handleScrollableHeight : 0;

     setScrollHandleHeight(newHandleHeight);
     setScrollHandlePosition(newHandlePosition);
   }
 }, [isCustomScrollEnabled]); // Depend on isCustomScrollEnabled
 ...
 useEffect(() => {
   ...
   if (container && isCustomScrollEnabled) {
     ...
     // Initial update and update on resize/content change
     updateScrollbar();

     // Observe content changes to update scrollbar (e.g., images loading)
     ...
     // If custom scroll is disabled, ensure native scrollbar is visible
     container.style.overflowY = 'auto';
   }
 }, [isCustomScrollEnabled, updateScrollbar, displayFiles]); // Depend on displayFiles to update when content changes

 // Handle drag start on custom scrollbar handle
 const handleDragStart = useCallback((e) => {
   if (!isCustomScrollEnabled) return;
   setIsDragging(true);
   dragStartY.current = e.clientY || e.touches[0].clientY;
   scrollStartY.current = fileListContainerRef.current.scrollTop;
   e.preventDefault(); // Prevent default text selection etc.
 }, [isCustomScrollEnabled]);

 // Handle drag move
 const handleDragMove = useCallback((e) => {
   if (!isDragging || !isCustomScrollEnabled) return;
   ...
   const handleScrollableHeight = trackHeight - scrollHandleHeight;

   if (scrollableHeight > 0 && handleScrollableHeight > 0) {
     // Calculate new scroll top based on drag movement relative to track height
     const scrollDelta = (deltaY / handleScrollableHeight) * scrollableHeight;
     container.scrollTop = scrollStartY.current + scrollDelta;
   }
 }, [isDragging, isCustomScrollEnabled, scrollHandleHeight]);
 ...
 useEffect(() => {
   if (isDragging) {
     ...
     document.addEventListener('touchmove', handleDragMove, { passive: false }); // Use passive: false for touchmove
     document.addEventListener('touchend', handleDragEnd);
     document.addEventListener('touchcancel', handleDragEnd); // Handle touch cancel
 ...
 // Toggle custom scrollbar and disable pagination
 const toggleCustomScroll = () => {
   setIsCustomScrollEnabled(prev => {
     const newState = !prev;
     if (newState) {
       setIsPaginationEnabled(false); // Disable pagination if custom scroll is enabled
     } else {
       // Optional: Re-enable pagination when custom scroll is disabled
 ...
 const toggleSelectAll = () => {
   // Determine the list of files relevant to 'Select All' based on pagination and custom scroll state
   const filesToConsider = isPaginationEnabled && !isCustomScrollEnabled ?
   ...
   <div className={cn(
     ...
     <div className="flex flex-col md:flex-row gap-3 md:gap-4 mb-6 items-center justify-between flex-wrap">
       ...
       <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
         {/* Pagination Toggle */}
         <button
           onClick={() => setIsPaginationEnabled(prev => {
             const newState = !prev;
             if (newState) {
               setIsCustomScrollEnabled(false); // Disable custom scroll if pagination is enabled
             }
             return newState;
           })}
           className={cn(
             ...
             isCustomScrollEnabled && 'opacity-50 cursor-not-allowed' // Dim if custom scroll is on
           )}
           ...
           disabled={isCustomScrollEnabled} // Disable if custom scroll is on
         >
         ...
         <button
           onClick={toggleCustomScroll}
           className={cn(
             'p-2 rounded-md transition-colors duration-200',
             isCustomScrollEnabled
               ? 'bg-blue-600 text-white hover:bg-blue-700'
               : darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-600 hover:bg-gray-300',
             isLoading && 'opacity-50 cursor-not-allowed' // Dim if loading
           )}
           aria-label={isCustomScrollEnabled ? "Disable custom scroll" : "Enable custom scroll"}
           aria-pressed={isCustomScrollEnabled}
           title={isCustomScrollEnabled ? "Disable Custom Scroll" : "Enable Custom Scroll"}
           disabled={isLoading} // Disable if loading
         >
           <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
             {/* Icon representing a scrollbar */}
             <path strokeLinecap="round" strokeLinejoin="round" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 6h2m-2 0h2m-2 0h2M19 17V7m0 10a2 2 0 01-2 2h-2a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2" />
           </svg>
         </button>
           ...
           {showSortOptions && (
             <div
               ...
               className={cn(
                 ...
                 'max-h-[40vh] sm:max-h-[75vh] overflow-y-auto', // Added overflow-y-auto for long lists
       ...
       <div className={cn(
         ...
         <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
           ...
           <div className="w-full md:w-auto">
             <button
               ...
               className={cn(
                 ...
                 (isPaginationEnabled && !isCustomScrollEnabled ? displayFiles.every(file => selectedFiles.includes(file._id)) && displayFiles.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0)
             ...
             >
               {(isPaginationEnabled && !isCustomScrollEnabled ? displayFiles.every(file => selectedFiles.includes(file._id)) && displayFiles.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0) ? 'Deselect All' : 'Select All'}
             </button>
           </div>
           ...
           <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
             {selectedFiles.length > 0 && isPaginationEnabled && !isCustomScrollEnabled ? `${selectedFiles.length} of ${sortedFiles.length} total items selected till this page.`
               : `${selectedFiles.length} of ${isPaginationEnabled && !isCustomScrollEnabled ? displayFiles.length : sortedFiles.length} selected.`
             }
           </div>
     ...
     {/* Added relative positioning to contain the absolute custom scrollbar */}
     <div className="min-h-[250px] relative">
       {isLoading ? (
         ...
         <div
           ref={fileListContainerRef} // Attach ref to the scrollable container
           className={cn(
             'grid gap-4 pr-2', // Added pr-2 for padding on the right for the native scrollbar space
             view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1',
             // Hide native scrollbar if custom scroll is enabled, otherwise show auto
             isCustomScrollEnabled ? 'overflow-y-hidden' : 'overflow-y-auto',
             'max-h-[60vh] transition-all duration-300 ease-in-out' // Added max-height and transition
           )}
           // Added tabIndex for keyboard scrolling if needed, though not strictly required for mouse/touch drag
           tabIndex={isCustomScrollEnabled ? 0 : -1}
         >
           {/* Render File Items */}
       ...
       {/* Custom Scrollbar */}
       {isCustomScrollEnabled && displayFiles.length > 0 && !isLoading && (
         <div
           ref={customScrollbarRef} // Attach ref to the scrollbar track
           className={cn(
         ...
         >
           <div
             className={cn(
               ...
               isDragging && (darkMode ? 'bg-blue-400' : 'bg-blue-700') // Change color while dragging
             )}
             style={{
               height: `${scrollHandleHeight}px`,
               transform: `translateY(${scrollHandlePosition}px)`,
             }}
             onMouseDown={handleDragStart}
             onTouchStart={handleDragStart} // Add touch support
           />
         </div>
       )}
     </div>
     ...
     {/* Hide pagination controls if custom scroll is enabled */}
     {isPaginationEnabled && !isCustomScrollEnabled && sortedFiles.length > 0 && (
       <div className={cn(
     ...
     {/* Message when custom scroll is enabled but no files */}
     {isCustomScrollEnabled && sortedFiles.length === 0 && (
       <div className={cn("text-center mt-4 text-sm", darkMode ? "text-gray-500" : "text-gray-400")}>
     ...
     <style jsx>{`
       ...
       /* Hide native scrollbar for the container when custom scroll is enabled */
