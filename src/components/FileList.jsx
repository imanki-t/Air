    ...
    <div className="flex flex-col md:flex-row gap-3 md:gap-4 mb-6 items-center justify-between flex-wrap">
      ...
      <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
          ...
          >
           ...
           {/* Custom Scrollbar Toggle Button */}
          <button
          ...
          >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   {isCustomScrollbarEnabled ? (
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 15l-2 2m0 0l-2-2m2 2V9m2.003 2.002H17m-1.001-.001h-1.002m2.002-.002V9.999M17 12a5 5 0 11-10 0 5 5 0 0110 0z" /> // Scrollbar icon with arrow pointing up
                   ) : (
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8 7h.01M12 7h.01M16 7h.01M9 13h.01M15 13h.01M12 17h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /> // List icon
                   )}
               </svg>
          </button>
...
onClick={() => setShowSortOptions(prev => !prev)}
    ...
    <div className={cn("min-h-[250px]", {
       'overflow-y-auto custom-scrollbar': isCustomScrollbarEnabled
    })}>
      {/* Files Display Area */}
      {isLoading ? (
        <div className={cn(
        ...
        <div className={cn(
          ...
          {/* Render File Items */}
          {(isPaginationEnabled ? paginatedFiles : sortedFiles).map(file => (
            <FileItem
              key={file._id}
              file={file}
              darkMode={darkMode}
    ...
    {showBatchDownloadProgress && (
      <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fadeIn backdrop-blur-sm">
        <div
          ...
          className={cn(
            'p-6 rounded-lg shadow-xl max-sm w-full border animate-modalIn',
            darkMode ? 'bg-gray-800 border-gray-700 text-gray-200' : 'bg-white border-gray-200 text-gray-800'
          )}
  ...
  <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
    ...
    <style jsx>{`
      ...
      .custom-scrollbar::-webkit-scrollbar-track {
          background: ${darkMode ? '#4b5563' : '#e5e7eb'}; /* Track color based on theme */
          border-radius: 5px;
      }

      .custom-scrollbar::-webkit-scrollbar-thumb {
          background: ${darkMode ? '#6b7280' : '#9ca3af'}; /* Thumb color based on theme */
          border-radius: 5px;
          border: 2px solid ${darkMode ? '#4b5563' : '#e5e7eb'}; /* Border for thumb */
          cursor: grab; /* Indicate it's draggable */
      }

      .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background: ${darkMode ? '#9ca3af' : '#6b7280'}; /* Darker thumb on hover */
      }
      ...
      .custom-scrollbar::-webkit-scrollbar-thumb:active {
       ...
       /* Optional: Hide default scrollbar on other browsers if needed (less universal) */
      .custom-scrollbar {
        scrollbar-width: thin; /* "auto" or "thin" */
        scrollbar-color: ${darkMode ? '#6b7280' : '#9ca3af'} ${darkMode ? '#4b5563' : '#e5e7eb'}; /* thumb track */
      }
