 ...
 return (
   ...
   >
         ...
         <ul className="space-y-2 max-h-60 overflow-y-auto pr-2">
           {files.map((file, index) => (
             <li
               ...
               className={cn(
                 'flex justify-between items-center p-3 rounded-md shadow-sm w-full',
             ...
             >
               ...
               <button
                 ...
                 className={cn(
                   "ml-4 p-1.5 rounded-full transition-colors flex-shrink-0",
     ...
     {isUploading && uploadFileRef.current && (
       <div className="mb-4 p-4 rounded-lg shadow-inner border border-transparent">
         <h4 className={cn('text-lg font-semibold mb-2', darkMode ? 'text-white' : 'text-gray-800')}>
           Uploading: {getTruncatedFileName(uploadFileRef.current.name)} {isMultipleFilesSelected ? `(${currentFileIndex + 1} of ${files.length})` : ''}
         </h4>
     ...
     {files.length > 0 && !isUploading && (
       <div className="flex gap-3 mt-2">
         ...
         >
           Upload {isMultipleFilesSelected ? `(${files.length})` : ''}
         </button>
         <button
           type="button"
           onClick={() => {
         ...
         >
           Clear {isSingleFileSelected ? '' : 'All'}
         </button>
       </div>
     )}
       ...
       <div className="mt-6">
         ...
         <ul className="space-y-2 max-h-40 overflow-y-auto pr-2">
           {uploadedFiles.map((file, index) => (
             <li
               key={file.name + file.size + index}
               className={cn('flex justify-between items-center p-3 rounded-md shadow-sm w-full', darkMode ? 'bg-green-800/40 text-green-200' : 'bg-green-50 text-green-700')}
             >
