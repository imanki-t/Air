...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
  ...
  const [scrollbarThumbPosition, setScrollbarThumbPosition] = useState(0); // Position in pixels
  ...
  const scrollbarThumbRef = useRef(null); // Ref for the scrollbar thumb
  ...
  const updateScrollbarThumbPosition = useCallback(() => {
    ...
    if (container && track) {
      ...
      const thumbHeight = scrollbarThumbRef.current?.clientHeight || 0;
      const trackHeight = track.clientHeight;
      const thumbPosition = scrollPercentage * (trackHeight - thumbHeight);
      setScrollbarThumbPosition(thumbPosition);
    }
  }, []);
  ...
  useEffect(() => {
    const container = fileListContainerRef.current;
    if (container) {
      container.addEventListener('scroll', updateScrollbarThumbPosition);
      // Initial position update
      updateScrollbarThumbPosition();
      // Update position on resize (in case scrollable area changes)
      window.addEventListener('resize', updateScrollbarThumbPosition);
    }

    return () => {
      if (container) {
        container.removeEventListener('scroll', updateScrollbarThumbPosition);
      }
      window.removeEventListener('resize', updateScrollbarThumbPosition);
    };
  }, [updateScrollbarThumbPosition, paginatedFiles, sortedFiles]); // Depend on file list changes
  ...
  const handleThumbMouseMove = useCallback((e) => {
    if (!isDraggingThumb || !fileListContainerRef.current || !scrollbarTrackRef.current) return;
    ...
    let newThumbPosition = e.clientY - trackRect.top - thumbHeight / 2;

    // Clamp the new position to stay within the track bounds
    newThumbPosition = Math.max(0, Math.min(newThumbPosition, track.clientHeight - thumbHeight));

    // Calculate the corresponding scroll position in the container
    const scrollPercentage = newThumbPosition / (track.clientHeight - thumbHeight);
    const newScrollTop = scrollPercentage * (container.scrollHeight - container.clientHeight);

    container.scrollTop = newScrollTop;
    setScrollbarThumbPosition(newThumbPosition);
  ...
  }, [isDraggingThumb, handleThumbMouseMove]);

  // Keyboard Scrolling
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (fileListContainerRef.current && !isEditingPage) { // Only scroll if not editing page number
        const container = fileListContainerRef.current;
    ...
    // Add listener to the document to capture key presses anywhere
    document.addEventListener('keydown', handleKeyDown);

    return () => {
      document.removeEventListener('keydown', handleKeyDown);
    };
  }, [isEditingPage]); // Re-run effect if isEditingPage changes


  // Calculate scrollbar thumb size
  const scrollbarThumbHeight = useCallback(() => {
    const container = fileListContainerRef.current;
    const track = scrollbarTrackRef.current;
    if (container && track && container.scrollHeight > container.clientHeight) {
      // Thumb height is proportional to the visible content height
      const percentageVisible = container.clientHeight / container.scrollHeight;
      const minHeight = 20; // Minimum thumb height
      const height = track.clientHeight * percentageVisible;
      return Math.max(minHeight, height);
    }
    return 0; // Hide thumb if not scrollable
  }, []);
    ...
    <div className={cn(
      ...
      <div className="min-h-[250px] flex"> {/* Added flex to accommodate scrollbar */}
        ...
        <div
          ...
          className={cn(
            'flex-grow pr-2 overflow-y-auto', // Added pr-2 for spacing when scrollbar is visible
            // Hide native scrollbar - Use utility class
            'scrollbar-hide'
          )}
          style={{
        ...
        {showCustomScrollbar && fileListContainerRef.current && fileListContainerRef.current.scrollHeight > fileListContainerRef.current.clientHeight && (
          <div
            ref={scrollbarTrackRef}
            className={cn(
              'w-3 ml-2 rounded-full relative', // Adjusted width and added spacing, relative for thumb absolute positioning
              darkMode ? 'bg-gray-700' : 'bg-gray-300' // Scrollbar track color
            )}
          >
            {/* Scrollbar Thumb (Draggable Dot) */}
            <div
              ref={scrollbarThumbRef}
              className={cn(
                'absolute left-1/2 transform -translate-x-1/2 w-4 rounded-full cursor-pointer', // Center the thumb, cursor indicates draggable
                darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-green-600 hover:bg-green-500' // Theme colors
              )}
              style={{
                height: scrollbarThumbHeight(), // Dynamic height
                top: scrollbarThumbPosition, // Dynamic position
              }}
              onMouseDown={handleThumbMouseDown}
              onTouchStart={handleThumbMouseDown} // Add touch support
              // MouseUp and MouseLeave listeners are on the document for more reliable dragging
            >
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fadeIn backdrop-blur-sm">
          ...
          >
            ...
            <p
              ...
              Creating ZIP archive ({selectedFiles.length} {selectedFiles.length !== 1 ? 'items' : 'item'}).</p>
          </div>
        </div>
      )}
