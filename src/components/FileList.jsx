...
// New state for custom scrollbar
...
// Ref for the custom scrollbar thumb
...
setShowSortOptions(false);
    ...
    if (isEditingPage &&
pageInputRef.current && !pageInputRef.current.contains(e.target)) {
...
useEffect(() => {
   ...
   if (!container || !isCustomScrollbarEnabled) return;

   const handleScroll = () => {
       const scrollPercentage = container.scrollTop / (container.scrollHeight - container.clientHeight);
       setScrollbarThumbPosition(scrollPercentage * (container.clientHeight - (scrollbarThumbRef.current?.offsetHeight || 0)));
   };
...
}, [isCustomScrollbarEnabled, paginatedFiles]); // Recalculate on pagination change

const handleScrollbarThumbMouseDown = (e) => {
   if (!isCustomScrollbarEnabled) return;
   setIsDraggingScrollbar(true);
   setStartDragY(e.clientY);
   setStartScrollTop(fileListContainerRef.current.scrollTop);
   e.preventDefault(); // Prevent default drag behavior
};

useEffect(() => {
   const handleMouseMove = (e) => {
       if (!isDraggingScrollbar || !fileListContainerRef.current) return;
       ...
       // Calculate how much the content should scroll for the thumb movement
       const scrollRatio = (contentHeight - visibleHeight) / (scrollbarTrackHeight - thumbHeight);
       const newScrollTop = startScrollTop + deltaY * scrollRatio;

       // Apply the new scroll position, clamping within bounds
       fileListContainerRef.current.scrollTop = Math.max(0, Math.min(newScrollTop, contentHeight - visibleHeight));
   };
...
}, [isDraggingScrollbar, startDragY, startScrollTop, isCustomScrollbarEnabled]);
...
11-18 0 9 9 0 0118 0z" /> // Plus icon for enabling
           ...
           >
               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   <path strokeLinecap="round" strokeLinejoin="round" d="M9 4h6m-6 0v16m0-16H5a2 2 0 00-2 2v10a2 2 0 002 2h4m6-16v16m0-16h4a2 2 0 012 2v10a2 2 0 01-2 2h-4M9 20h6" />
               </svg>
           </button>
...
`Delete ${selectedFiles.length} items` : "Select files to delete"}
   ...
   <div className={cn("min-h-[250px] relative", isCustomScrollbarEnabled && "custom-scroll-container")}>
     <div
         ref={fileListContainerRef}
         className={cn(
           'grid gap-4',
           view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1',
            isCustomScrollbarEnabled ? "custom-scroll-content" : "overflow-y-auto" // Apply custom scroll or default
         )}
      >
      {isLoading ?
(
...
'Try adjusting your search or filter.' : 'Upload some files!'}
    ...
    {isCustomScrollbarEnabled && sortedFiles.length > 0 && (
        <div className={cn("custom-scrollbar-track", darkMode ? "bg-gray-700" : "bg-gray-300")}>
            {/* Custom Scrollbar Thumb */}
            <div
                ref={scrollbarThumbRef}
                className={cn(
                    "custom-scrollbar-thumb",
                    isDraggingScrollbar ? "bg-blue-700" : (darkMode ? "bg-blue-600 hover:bg-blue-500" : "bg-blue-500 hover:bg-blue-600")
                )}
                style={{ transform: `translateY(${scrollbarThumbPosition}px)` }}
...
clearTimout(e.currentTarget.dataset.pressTimer);
}}
                      onMouseDown={(e) => {
...
<style jsx>{`
      ...
      .animate-modalIn { animation: modalIn 0.25s ease-out;
       ...
       .custom-scroll-content {
           ...
           padding-right: 15px; /* Make space for the custom scrollbar */
            /* Hide default scrollbar for Webkit browsers */
           scrollbar-width: none;  /* Hide default scrollbar for Firefox */
            -ms-overflow-style: none;  /* Hide default scrollbar for IE 10+ */
       }
       ...
       .custom-scrollbar-thumb {
           ...
           left: 0;
           width: 100%;
           height: 40px; /* Thumb height - adjust as needed */
           border-radius: 4px;
           cursor: grab;
           /* Use theme colors */
            transition: background-color 0.2s ease-in-out;
