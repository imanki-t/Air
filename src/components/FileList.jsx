...
const filteredAndSearchedFiles = files
   .filter(f => filter === 'all' || f.metadata?.type === filter)
   .filter(f => f.filename.toLowerCase().includes(searchInput.toLowerCase()));

const totalPages = Math.ceil(filteredAndSearchedFiles.length / itemsPerPage);
const startIndex = (currentPage - 1) * itemsPerPage;
const endIndex = startIndex + itemsPerPage;

const filesToDisplayAndSort = isPaginationEnabled
   ? filteredAndSearchedFiles.slice(startIndex, endIndex)
   : filteredAndSearchedFiles;

const sortedFiles = [...filesToDisplayAndSort].sort((a, b) => {
 switch (sortOption) {
   case 'alphabetical': return a.filename.localeCompare(b.filename);
   case 'reverseAlphabetical': return b.filename.localeCompare(a.filename);
   case 'large': return (b.length || 0) - (a.length || 0);
   case 'small': return (a.length || 0) - (b.length || 0);
   case 'latest': return new Date(b.uploadDate) - new Date(a.uploadDate);
 ...
 <div className={cn(
   ...
   <div className="text-center mb-6">
     ...
     <span className={cn('text-sm px-3 py-1 rounded-full inline-block transition-all duration-200',
       ...
       {filteredAndSearchedFiles.length} item{filteredAndSearchedFiles.length !== 1 ? 's' : ''}{filter !== 'all' ? ` (${filter})` : ''}
     </span>
   </div>
...
<path strokeLinecap="round" strokeLinejoin="round" d="M21 16l-4 4m0 0l-4-4m4 4V8" />
           ...
           >
             <div>
                 ...
                 {[
                   ...
                   { label: 'Large Files', id: 'large' },
                   { label: 'Small Files', id: 'small' },
                 ].map(opt => (
                   <button
                     key={opt.id}
     ...
     <div className={cn(
       ...
       <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
         ...
         <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
            {selectedFiles.length > 0 && isPaginationEnabled ? `${selectedFiles.length} of ${filteredAndSearchedFiles.length} total items selected till this page.`
  ...
  {isPaginationEnabled && filteredAndSearchedFiles.length > 0 && (
      <div className={cn(
     ...
     {isPaginationEnabled && filteredAndSearchedFiles.length === 0 && !isLoading && (
