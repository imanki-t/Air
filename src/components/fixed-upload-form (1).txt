...
const UploadForm = ({ refresh, darkMode }) => {
  ...
  // Check for saved upload state on component mount only (not on focus)
  useEffect(() => {
  ...
  // This is no longer tied to beforeunload, preventing the dialog
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // Remove the event listener after upload completes
    ...
    if (droppedFile) {
      ...
      if (pendingResume) {
        if (pendingResume.fileName === droppedFile.name && 
          ...
          setMessage('Different file selected. Starting new upload.');
          setPendingResume(null);
          localStorage.removeItem(STORAGE_KEY);
        }
      }
    ...
    >
      ...
      {file && !isUploading && (
        <div className="flex gap-3 mt-2">
          <button
            type="submit"
            className="flex-1 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-medium transition-colors"
            disabled={pendingResume && message.includes('Different file')}
          >
      ...
      {message && (
        <p className={`mt-4 text-center font-medium text-base ${
          message.includes('successfully') || message.includes('verified') || message.includes('Ready to resume')
