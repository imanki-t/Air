...
const getThumbnail = async (req, res) => {
  ...
  try {
    ...
    if (isImage) {
      // Handle image thumbnails with Sharp
      try {
      ...
      // Handle video thumbnails with FFmpeg
      try {
        // Create temporary files for video processing
        const tempDir = os.tmpdir();
        const fileExtension = path.extname(fileMetadata.data.name) || '.mp4';
        
        tempInputFile = path.join(tempDir, `temp_video_${Date.now()}${fileExtension}`);
        tempOutputFile = path.join(tempDir, `temp_thumbnail_${Date.now()}.jpg`);

        // Write the video buffer to a temporary file
        await fs.writeFile(tempInputFile, fileBuffer);

        // Generate thumbnail using FFmpeg
        await generateVideoThumbnail(tempInputFile, tempOutputFile);

        // Read the generated thumbnail
        const thumbnailFileBuffer = await fs.readFile(tempOutputFile);

        // Process with Sharp to ensure consistent sizing and format
        thumbnailBuffer = await sharp(thumbnailFileBuffer)
          .resize(300, 300, {
            fit: 'inside',
    ...
    // Try to fall back to serving original file with cache headers for images only
    if (error.message && !error.message.includes('FFmpeg')) {
      try {
        ...
        // Only fallback for images, not videos
