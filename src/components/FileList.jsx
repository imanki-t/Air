...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
  ...
  const initialThumbY = useRef(0); // To store initial thumb position during drag
  const initialScrollTop = useRef(0); // To store initial scroll position during drag
  ...
  useEffect(() => {
    ...
    if (fileListContainer && scrollbarTrack) {
      const contentHeight = fileListContainer.scrollHeight;
      const visibleHeight = fileListContainer.clientHeight;
      const trackHeight = scrollbarTrack.clientHeight;

      const thumbHeight = Math.max(
        (visibleHeight / contentHeight) * trackHeight,
        20 // Minimum thumb height (e.g., 20px)
      );
      setThumbHeight(thumbHeight);

      // Calculate thumb position based on scroll position
      const scrollPercentage = fileListContainer.scrollTop / (contentHeight - visibleHeight);
      const thumbTop = scrollPercentage * (trackHeight - thumbHeight);
      setThumbTop(thumbTop);
    }
  }, [files, paginatedFiles, view, showCustomScrollbar]); // Recalculate when files, view, or scrollbar visibility changes
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (scrollbarTrackRef.current && fileListContainer) {
        const contentHeight = fileListContainer.scrollHeight;
        const visibleHeight = fileListContainer.clientHeight;
        ...
        const scrollPercentage = fileListContainer.scrollTop / (contentHeight - visibleHeight);
        const thumbHeight = scrollbarThumbRef.current.clientHeight; // Use current thumb height
        const thumbTop = scrollPercentage * (trackHeight - thumbHeight);
        setThumbTop(thumbTop);
      }
    };

    if (fileListContainer && showCustomScrollbar) {
  ...
  }, [showCustomScrollbar]); // Add/remove listener when scrollbar visibility changes

  // Drag handlers
  const handleMouseDown = (e) => {
    e.preventDefault(); // Prevent text selection etc.
    setIsDragging(true);
    initialThumbY.current = e.clientY || e.touches[0].clientY;
    initialScrollTop.current = fileListContainerRef.current.scrollTop;
  ...
  const handleMouseMove = (e) => {
    ...
    if (!fileListContainer || !scrollbarTrack) return;

    const currentClientY = e.clientY || e.touches[0].clientY;
    const deltaY = currentClientY - initialThumbY.current;
    ...
    if (contentHeight <= visibleHeight || trackHeight <= thumbHeight) {
        return;
    }

    // Calculate how much scrolling should happen based on thumb movement
    // deltaY / (trackHeight - thumbHeight) = deltaScrollTop / (contentHeight - visibleHeight)
    const scrollDelta = (deltaY / (trackHeight - thumbHeight)) * (contentHeight - visibleHeight);

    // Calculate new scroll position
    let newScrollTop = initialScrollTop.current + scrollDelta;

    // Clamp scroll position within bounds
    newScrollTop = Math.max(0, Math.min(newScrollTop, contentHeight - visibleHeight));

    // Update the actual scroll position of the file list container
    fileListContainer.scrollTop = newScrollTop;

    // The scroll event listener will handle updating the thumb position (setThumbTop)
  };

  const handleMouseUp = () => {
    setIsDragging(false);
    initialThumbY.current = 0;
  ...
  }, [isDragging]); // Dependency array includes isDragging to ensure listeners are correctly managed
      ...
      <div className="min-h-[250px] flex"> {/* Added flex to accommodate scrollbar */}
        ...
        <div
          ...
          className={cn(
            ...
            showCustomScrollbar ? 'overflow-y-scroll' : 'overflow-y-auto', // Conditionally control overflow
        ...
        {showCustomScrollbar && sortedFiles.length > 0 && ( // Only show if toggle is on and there are files
          <div
            ...
            className={cn(
              'w-3 ml-2 rounded-full relative overflow-hidden cursor-pointer', // Increased width, added overflow-hidden and cursor
          ...
          >
            <div
              ...
              className={cn(
                'absolute w-full rounded-full touch-action-none', // Added touch-action-none for better mobile drag
                 isDragging ? 'bg-blue-500' : (darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-green-600 hover:bg-green-500'), // Thumb color and hover
