...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
  ...
  // Modify the Initial File Fetch (usually inside useEffect)
  useEffect(() => {
    const fetchFiles = async () => {
      const token = getToken();
      if (!token) {
        alert('Authentication required to fetch files.');
        return;
      }
      ...
      try {
        ...
        // Optional: handle 401/403 errors
      }
    };

    fetchFiles();
  }, []); // Added refresh dependency
  ...
  const batchShare = async () => {
    ...
    try {
      ...
      for (const file of filesToZip) {
        console.log(`Workspaceing file: ${file.filename} (${file._id})`); // Debug log
            ...
            {showSortOptions && (
              ...
              >
                ...
                <div>
                    ...
                    {[
                      { label: 'Default (Latest)', id: 'default' }, // Changed label
                      { label: 'Name', id: 'name' },
                      { label: 'Size', id: 'size' },
                      { label: 'Date', id: 'date' },
                    ].map(opt => (
