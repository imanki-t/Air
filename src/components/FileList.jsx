...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
 ...
 const [showCustomScrollbar, setShowCustomScrollbar] = useState(false); // New state for custom scrollbar
 ...
 const fileListContainerRef = useRef(null); // Ref for the file list scroll container
 ...
 useEffect(() => {
   ...
   if (!container || !thumb || !showCustomScrollbar) return;
   ...
   const handleMouseDown = (e) => {
     ...
     setScrollStartPos(container.scrollTop);
     thumb.classList.add('dragging'); // Add dragging class for styling
   };
   ...
   const handleMouseMove = (e) => {
     ...
     // Calculate how much the content should scroll based on thumb movement
     const scrollDelta = dragDelta * (container.scrollHeight / container.clientHeight);

     container.scrollTop = scrollStartPos + scrollDelta;
   };

   const handleMouseUp = () => {
     setIsDraggingScrollbar(false);
     thumb.classList.remove('dragging'); // Remove dragging class
   };
   ...
   // Initial thumb position calculation
   updateThumbPosition();
   ...
   return () => {
     ...
     thumb.classList.remove('dragging'); // Ensure class is removed on cleanup
   };
 }, [showCustomScrollbar, isDraggingScrollbar, dragStartPos, scrollStartPos, paginatedFiles, sortedFiles]); // Re-run if file list or pagination changes

 // Update thumb position when pagination changes or files load
 useEffect(() => {
   const container = fileListContainerRef.current;
   const thumb = scrollbarThumbRef.current;
   if (container && thumb && showCustomScrollbar) {
       const updateThumbPosition = () => {
           const scrollPercentage = container.scrollTop / (container.scrollHeight - container.clientHeight);
           const thumbPosition = scrollPercentage * (container.clientHeight - thumb.clientHeight);
           thumb.style.transform = `translateY(${thumbPosition}px)`;
 ...
 }, [paginatedFiles, sortedFiles, showCustomScrollbar, isLoading]);
   ...
   <div className={cn(
     ...
     <div className="flex flex-col md:flex-row gap-3 md:gap-4 mb-6 items-center justify-between flex-wrap">
       ...
       <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
         ...
         {/* Toggle Custom Scrollbar (New Button) */}
         <button
         ...
         >
           <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
               ...
               <path strokeLinecap="round" strokeLinejoin="round" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10h2m-2 0h2m2 0h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2v10m-2 0h2m-2 0h2" />
           </svg>
         </button>
 ...
 onClick={() => setShowSortOptions(prev => !prev)}
     ...
     {/* Added relative positioning for the scrollbar container */}
     <div className={cn("relative", showCustomScrollbar ? "pr-3" : "")}> {/* Add right padding when scrollbar is visible */}
         <div
             ref={fileListContainerRef}
             className={cn(
                 "min-h-[250px] overflow-y-auto",
                 showCustomScrollbar ? "custom-scrollbar" : "" // Apply custom scrollbar styles
             )}
         >
             {isLoading ? (
                 <div className={cn(
         ...
         {showCustomScrollbar && (
             <div className={cn(
                 "absolute top-0 right-0 w-2 h-full", // Scrollbar track container
                 darkMode ? "bg-gray-700/50" : "bg-gray-200/50" // Track background
             )}>
                 <div
                     ref={scrollbarThumbRef}
                     className={cn(
                         "absolute top-0 left-0 w-full rounded-full cursor-pointer transition-colors duration-200",
                         "h-16", // Initial thumb height (can be adjusted based on content/container ratio)
                         darkMode ? "bg-blue-500 hover:bg-blue-400" : "bg-blue-600 hover:bg-blue-700", // Thumb color
                         "active:bg-blue-800" // Active state color
                     )}
                     style={{ height: '64px' }} // Default height, will be adjusted by JS
                 >
                     {/* Optional: Add a small handle/indicator on the thumb */}
                     <div className={cn(
                         "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1 h-4 rounded-full",
                  ...
                  />
                   ...
                   <span
                       ...
                       onMouseDown={(e) => {
                           ...
                           if (e.button === 0) {
                                // Store timer ID directly on the element
                               clearTimeout(e.currentTarget.dataset.pressTimer); // Clear any previous timer
                               e.currentTarget.dataset.pressTimer = setTimeout(() => {
                                 handlePageClick();
                               }, 3000); // 3 seconds for long press
                           }
 ...
 <div className="mt-4 text-center">
   <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
     ...
     <style jsx>{`
       ...
       /* Style for the custom thumb when dragging */
       .custom-scrollbar + div > div.dragging {
           opacity: 0.8; /* Example: Make thumb slightly transparent when dragging */
       }
     `}</style>
   </div>
 );
