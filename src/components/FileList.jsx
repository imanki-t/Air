...
// ─────────────────────────────────────────────────────────────────────────────
const FileList = ({ files = [], refresh, onQuickRefresh, darkMode, isLoading, folders = [], onFoldersChanged, hideFolderFiles = false }) => {
  ...
  // Live file/folder updates are handled by a single shared socket connection
  // owned by App.jsx (patches files state directly + calls onFoldersChanged),
  // so this component doesn't need its own connection.
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e) => {
      ...
      // 5. Refresh Files: 'r'
      // Bandwidth-friendly: only re-fetch the files currently on screen (the
      // active page), or the 50 most recent files when pagination is off,
      // instead of re-downloading metadata/icons for the whole library.
      if (key === 'r' && !isCmdOrCtrl) {
        e.preventDefault();
        const idsToRefresh = (isPaginationEnabled ? paginatedFiles : sortedFiles.slice(0, 50))
          .map((f) => f._id)
          .filter(Boolean);
        if (idsToRefresh.length > 0 && onQuickRefresh) {
          onQuickRefresh(idsToRefresh);
        } else {
          refresh?.();
        }
        return;
  ...
  }, [searchInput, showSortOptions, showFolderPicker, showDeleteConfirmModal, showBatchShareModal, selectionMode, selectedFiles, files, refresh, onQuickRefresh, filter, sortOption, currentPage, itemsPerPage, isPaginationEnabled, hideFolderFiles, folders]);
