...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
  ...
  const [isCustomScrollbarEnabled, setIsCustomScrollbarEnabled] = useState(false); // New state for custom scrollbar toggle
  const [scrollThumbHeight, setScrollThumbHeight] = useState(0); // State for the scroll thumb height
  const [scrollThumbTop, setScrollThumbTop] = useState(0); // State for the scroll thumb position
  const [isDragging, setIsDragging] = useState(false); // State for dragging the scroll thumb
  const [dragStartPos, setDragStartPos] = useState(0); // State for the initial drag position
  const [dragStartScrollTop, setDragStartScrollTop] = useState(0); // State for the initial scroll position during drag
  ...
  const fileListContainerRef = useRef(null); // Ref for the file list container
 ...
 useEffect(() => {
  ...
  setShowSortOptions(false);
      ...
      // Close scrollbar on outside click if it's not being dragged
      if (isCustomScrollbarEnabled && customScrollbarRef.current && !customScrollbarRef.current.contains(e.target) && !isDragging) {
          // setIsCustomScrollbarEnabled(false); // Decide if you want to disable on outside click
      }
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [batchOperationLoading, isEditingPage, isDragging]); // Add isDragging to dependencies
  ...
  useEffect(() => {
      const updateScrollbar = () => {
          ...
          if (container && isCustomScrollbarEnabled) {
              ...
              const thumbHeight = Math.max(20, (clientHeight / scrollHeight) * clientHeight); // Minimum thumb height
      ...
      // Handle drag events outside the component (on document)
      const handleMouseMove = (e) => {
          if (!isDragging) return;
          const scrollbarTrack = customScrollbarRef.current;
          const container = fileListContainerRef.current;
          if (scrollbarTrack && container) {
              const trackRect = scrollbarTrack.getBoundingClientRect();
              const dragDelta = e.clientY - dragStartPos;
              const scrollTrackHeight = trackRect.height - scrollThumbHeight;
              const scrollRange = container.scrollHeight - container.clientHeight;
              const newScrollTop = dragStartScrollTop + (dragDelta / scrollTrackHeight) * scrollRange;
  ...
  }, [isCustomScrollbarEnabled, isDragging, scrollThumbHeight, dragStartPos, dragStartScrollTop]); // Dependencies

  const handleThumbMouseDown = (e) => {
      e.preventDefault();
      setIsDragging(true);
      setDragStartPos(e.clientY);
      setDragStartScrollTop(fileListContainerRef.current.scrollTop);
  };

  const handleScrollbarTrackClick = (e) => {
      const scrollbarTrack = customScrollbarRef.current;
      const container = fileListContainerRef.current;
      if (scrollbarTrack && container && !isDragging) {
          const trackRect = scrollbarTrack.getBoundingClientRect();
          ...
          if (clickY < thumbCenter) {
              // Clicked above the thumb, scroll up by one "page" (container height)
              newScrollTop = container.scrollTop - container.clientHeight;
          } else {
              // Clicked below the thumb, scroll down by one "page" (container height)
              newScrollTop = container.scrollTop + container.clientHeight;
          }
  ...
  11-18 0 9 9 0 0118 0z" /> // Plus icon for enabling
            ...
            {/* Custom Scrollbar Toggle Button (New) */}
            <button
            ...
            >
                 {/* Replace with a suitable scrollbar icon */}
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 4H7a2 2 0 00-2 2v14a2 2 0 002 2h2m7-18h2a2 2 0 012 2v14a2 2 0 01-2 2h-2M9 4v16a2 2 0 002 2h2a2 2 0 002-2V4m-7 0h6" />
                 </svg>
            </button>
  ...
  `Delete ${selectedFiles.length} items` : "Select files to delete"}
      ...
      <div className="relative min-h-[250px] flex"> {/* Added flex and relative for positioning */}
          <div
              ref={fileListContainerRef}
              className={cn(
                  'w-full overflow-y-auto', // Allow vertical scrolling
                  isCustomScrollbarEnabled ? 'custom-scrollbar' : '', // Apply custom scrollbar class
                  view === 'grid' ? 'grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1 gap-4' // Grid/List view classes
              )}
          >
              {isLoading ? (
                  // Render Skeletons
          ...
          {isCustomScrollbarEnabled && sortedFiles.length > 0 && (
              <div
                  ...
                  className={cn(
                      'absolute right-0 top-0 bottom-0 w-2.5 rounded-full transition-colors duration-200',
                      darkMode ? 'bg-gray-700' : 'bg-gray-300' // Scrollbar track color
                  )}
                  onClick={handleScrollbarTrackClick} // Handle clicks on the track
              >
                  <div
                      ref={scrollThumbRef}
                      className={cn(
                          'absolute left-0 w-full rounded-full cursor-pointer',
                          darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-blue-500 hover:bg-blue-600' // Scrollbar thumb color
                      )}
                      style={{ height: scrollThumbHeight, transform: `translateY(${scrollThumbTop}px)` }}
                      onMouseDown={handleThumbMouseDown} // Handle dragging the thumb
                  />
              </div>
          )}
      </div>
  ...
  onTouchStart={(e) => {
                        ...
                        onMouseDown={(e) => {
                            ...
                            if (e.button === 0) {
                              ...
                              // Store timer ID directly on the element
                                clearTimeout(e.currentTarget.dataset.pressTimer); // Clear previous timer just in case
                                e.currentTarget.dataset.pressTimer = setTimeout(() => {

    handlePageClick();
                                }, 500); // Shorter press for editing page number
                            }
                        }}
                         onMouseUp={(e) => {
      ...
      {showDeleteConfirmModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fadeIn backdrop-blur-sm">
          <div
            ...
            className={cn('p-6 rounded-lg shadow-xl max-w-md w-full border animate-modalIn',
