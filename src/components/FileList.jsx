...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
 ...
 // New state for custom scroll
 const [isCustomScrollEnabled, setIsCustomScrollEnabled] = useState(false);
 ...
 // Ref for the file list container for custom scroll
 const fileListContainerRef = useRef(null);
 // Ref for the custom scroll thumb
 ...
 useEffect(() => {
   const fileList = fileListContainerRef.current;
   const scrollThumb = scrollThumbRef.current;

   if (!fileList || !scrollThumb || !isCustomScrollEnabled) return;

   const updateThumbPosition = () => {
     const scrollPercentage = (fileList.scrollTop / (fileList.scrollHeight - fileList.clientHeight)) * 100;
     const thumbPosition = (scrollPercentage / 100) * (fileList.clientHeight - scrollThumb.clientHeight);
     scrollThumb.style.transform = `translateY(${thumbPosition}px)`;
   };

   const updateScrollTop = (thumbPosition) => {
       const scrollPercentage = (thumbPosition / (fileList.clientHeight - scrollThumb.clientHeight)) * 100;
       fileList.scrollTop = (scrollPercentage / 100) * (fileList.scrollHeight - fileList.clientHeight);
   };
   ...
   const onMouseDown = (e) => {
       isDragging = true;
       startY = e.clientY;
       startScrollTop = fileList.scrollTop;
   ...
   const onMouseMove = (e) => {
       ...
       const thumbTrackHeight = fileList.clientHeight - scrollThumb.clientHeight;
       const scrollTrackHeight = fileList.scrollHeight - fileList.clientHeight;
       const newThumbPosition = Math.min(Math.max(0, parseFloat(scrollThumb.style.transform.replace('translateY(', '').replace('px)', '')) + deltaY), thumbTrackHeight);
       const scrollPercentage = newThumbPosition / thumbTrackHeight;
       fileList.scrollTop = scrollPercentage * scrollTrackHeight;
   ...
   fileList.addEventListener('scroll', updateThumbPosition);
   ...
   const handleResizeAndScroll = () => {
     if (!fileList || !scrollThumb) return;
     const contentHeight = fileList.scrollHeight;
     const containerHeight = fileList.clientHeight;
     const scrollbarHeight = containerHeight; // The track height is the container height

     // Calculate thumb size
     const thumbHeight = Math.max(containerHeight / contentHeight * scrollbarHeight, 20); // Minimum thumb height of 20px
   ...
   return () => {
     fileList.removeEventListener('scroll', updateThumbPosition);
     scrollThumb.removeEventListener('mousedown', onMouseDown);
     document.removeEventListener('mousemove', onMouseMove);
     document.removeEventListener('mouseup', onMouseUp);
     window.removeEventListener('resize', handleResizeAndScroll);
     document.body.classList.remove('select-none'); // Clean up body class
   };

 }, [isCustomScrollEnabled, paginatedFiles]); // Re-run effect if scroll mode changes or pagination changes
 ...
 px-3 py-1 rounded-full inline-block transition-all duration-200',
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /> // X icon for exit
           ...
           {/* Toggle Custom Scroll Bar (New Button) */}
           <button
               onClick={() => setIsCustomScrollEnabled(prev => !prev)}
               className={cn(
                   'p-2 rounded-md transition-colors duration-200',
               ...
               aria-label={isCustomScrollEnabled ? "Disable custom scroll bar" : "Enable custom scroll bar"}
               aria-pressed={isCustomScrollEnabled}
               title={isCustomScrollEnabled ? "Disable Custom Scroll Bar" : "Enable Custom Scroll Bar"}
           >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   {/* Icon representing a scroll bar - you can choose a better one */}
                   <path strokeLinecap="round" strokeLinejoin="round" d="M9 4H7a2 2 0 00-2 2v12a2 2 0 002 2h2m8-12h2a2 2 0 012 2v12a2 2 0 01-2 2h-2M9 4v16m8-16v16" />
               </svg>
           </button>
 ...
 >
  ...
  012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
   ...
   `Delete ${selectedFiles.length} items` : "Select files to delete"}
     ...
     {/* Files Display Area with min-height and custom scroll */}
       <div className={cn("min-h-[250px] relative", isCustomScrollEnabled ? 'pr-2' : '')}> {/* Added relative and padding-right */}
           {/* Files Display Area */}
           <div
               ...
               className={cn(
                  ...
                  isCustomScrollEnabled ? 'overflow-y-hidden' : 'overflow-y-auto' // Hide native scroll when custom is enabled
               )}
               style={{ maxHeight: '600px' }} // Set a max height to enable scrolling
           >
               {isLoading ? (
                   <div className={cn(
                       'grid gap-4',
   ...
   'Try adjusting your search or filter.' : 'Upload some files!'}
           ...
           {isCustomScrollEnabled && sortedFiles.length > 0 && (
               <div className={cn(
                   'absolute top-0 right-0 w-2 h-full rounded-full', // Scrollbar track
                   darkMode ? 'bg-gray-700' : 'bg-gray-300'
               )}>
                   <div
                       ref={scrollThumbRef}
                       className={cn(
                           'w-full rounded-full cursor-pointer transition-transform duration-100 ease-out', // Scrollbar thumb
                           darkMode ? 'bg-blue-500 hover:bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'
                       )}
                       style={{ height: '50px', transform: 'translateY(0px)' }} // Initial size and position (will be calculated by useEffect)
                   ></div>
               </div>
           )}
       </div>
