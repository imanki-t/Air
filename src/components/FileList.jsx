...
// New state for custom scrollbar visibility
...
// State for the dragger's position (optional, can calculate from scrollLeft)
...
// Ref for the scrollable file list container
const fileListContainerRef = useRef(null);
// Ref for the custom scrollbar dragger
const scrollbarDraggerRef = useRef(null);
// Ref for the custom scrollbar track
...
// Determine files to display based on pagination state
...
// Function to calculate dragger width and position
const calculateScrollbarMetrics = useCallback(() => {
   const container = fileListContainerRef.current;
   const track = scrollbarTrackRef.current;
   if (!container || !track) return { draggerWidth: 0, draggerLeft: 0 };

   const contentWidth = container.scrollWidth; // Total width of content
   const visibleWidth = container.clientWidth; // Visible width of the container
   const trackWidth = track.clientWidth; // Width of the scrollbar track

   // Calculate dragger width based on content vs visible width
   // Dragger width is proportional to the visible area
   const draggerWidth = Math.max(trackWidth * (visibleWidth / contentWidth), 20); // Minimum width of 20px

   // Calculate dragger position based on current scroll position
   // Dragger position is proportional to the scroll position
   const scrollRatio = container.scrollLeft / (contentWidth - visibleWidth);
   const draggerLeft = scrollRatio * (trackWidth - draggerWidth);

   return { draggerWidth, draggerLeft };
}, []);

// Update dragger position when the container scrolls
useEffect(() => {
   const container = fileListContainerRef.current;
   if (!container) return;

   const handleScroll = () => {
       // Update dragger position based on container's scrollLeft
       const { draggerLeft } = calculateScrollbarMetrics();
       if (scrollbarDraggerRef.current) {
           scrollbarDraggerRef.current.style.left = `${draggerLeft}px`;
       }
   };
...
}, [calculateScrollbarMetrics, isCustomScrollbarEnabled, isPaginationEnabled]); // Re-run effect if scrollbar state or pagination changes

// Handle mouse down on dragger
const handleMouseDown = (e) => {
   if (!isCustomScrollbarEnabled || isPaginationEnabled) return; // Only drag if enabled and pagination is off
   e.preventDefault();
   setIsDragging(true);
   setStartX(e.clientX);
   setScrollLeft(fileListContainerRef.current.scrollLeft);
   // Add global event listeners for mousemove and mouseup while dragging
   document.addEventListener('mousemove', handleMouseMove);
   document.addEventListener('mouseup', handleMouseUp);
};

// Handle touch start on dragger
const handleTouchStart = (e) => {
   ...
   setScrollLeft(fileListContainerRef.current.scrollLeft);
   // Add global event listeners for touchmove and touchend while dragging
...
const handleMouseMove = useCallback((e) => {
   if (!isDragging || !fileListContainerRef.current || !scrollbarTrackRef.current || !scrollbarDraggerRef.current) return;
   e.preventDefault();

   const dx = e.clientX - startX; // Change in mouse position
   const container = fileListContainerRef.current;
   const track = scrollbarTrackRef.current;
   const dragger = scrollbarDraggerRef.current;

   // Calculate the scroll amount based on dragger movement relative to track width
   const trackWidth = track.clientWidth;
   const visibleWidth = container.clientWidth;
   const contentWidth = container.scrollWidth;

   // Ensure contentWidth > visibleWidth before calculating scroll ratio
   if (contentWidth <= visibleWidth) return;

   // Calculate the maximum possible scrollLeft
   const maxScrollLeft = contentWidth - visibleWidth;

   // Calculate the scroll delta based on the dragger's movement
   // Dragger movement (dx) maps to scroll movement (scrollDelta)
   // scrollDelta / maxScrollLeft = dx / (trackWidth - dragger.clientWidth)
   const scrollDelta = dx * (maxScrollLeft / (trackWidth - dragger.clientWidth));

   // Calculate the new scroll position
   const newScrollLeft = scrollLeft + scrollDelta;

   // Apply the new scroll position, clamping it between 0 and maxScrollLeft
   container.scrollLeft = Math.max(0, Math.min(maxScrollLeft, newScrollLeft));

   // The scroll event listener on the container will update the dragger's visual position
}, [isDragging, startX, scrollLeft]); // Depend on isDragging, startX, and scrollLeft
...
const handleTouchMove = useCallback((e) => {
   ...
   const dx = e.touches[0].clientX - startX; // Change in touch position
   const container = fileListContainerRef.current;
   const track = scrollbarTrackRef.current;
   const dragger = scrollbarDraggerRef.current;
...
}, [handleTouchMove]); // Depend on handleTouchMove
...
}, [handleMouseMove, handleMouseUp, handleTouchMove, handleTouchEnd]); // Depend on the memoized handlers

// Update dragger width and position on mount and when files/view/size changes
useEffect(() => {
   if (isCustomScrollbarEnabled && !isPaginationEnabled) {
       const { draggerWidth, draggerLeft } = calculateScrollbarMetrics();
       if (scrollbarDraggerRef.current) {
           scrollbarDraggerRef.current.style.width = `${draggerWidth}px`;
           scrollbarDraggerRef.current.style.left = `${dragerLeft}px`; // Initial position
       }
   }
}, [filesToDisplay, view, isCustomScrollbarEnabled, isPaginationEnabled, calculateScrollbarMetrics]); // Re-calculate when files or view changes

// Handle click on the custom scrollbar track to jump to position
const handleTrackClick = (e) => {
   if (!isCustomScrollbarEnabled || isPaginationEnabled || isDragging || !fileListContainerRef.current || !scrollbarTrackRef.current || !scrollbarDraggerRef.current) return;
   ...
   const clickX = e.clientX - trackRect.left; // X position relative to the track

   const trackWidth = track.clientWidth;
   const draggerWidth = dragger.clientWidth;
   const contentWidth = container.scrollWidth;
   const visibleWidth = container.clientWidth;

   if (contentWidth <= visibleWidth) return;

   // Calculate the target scroll position based on the click position
   // Click position (clickX) maps to scroll position (targetScrollLeft)
   // targetScrollLeft / maxScrollLeft = clickX / trackWidth
   const maxScrollLeft = contentWidth - visibleWidth;
   let targetScrollLeft = (clickX / trackWidth) * maxScrollLeft;

   // Adjust targetScrollLeft so the dragger center moves to the click position
   // This is a simplified approach, could be improved
   const draggerCenterOffset = draggerWidth / 2;
   targetScrollLeft = targetScrollLeft - (draggerCenterOffset / trackWidth) * maxScrollLeft;
...
// Toggle Custom Scrollbar handler
const toggleCustomScrollbar = () => {
    if (isPaginationEnabled) {
        // If pagination is on, disable it and enable custom scrollbar
        setIsPaginationEnabled(false);
        setIsCustomScrollbarEnabled(true);
    } else {
        // If pagination is off, just toggle custom scrollbar
        setIsCustomScrollbarEnabled(prev => !prev);
    }
};
      ...
      <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
          ...
          <button
              onClick={() => {
                  ...
                  // If enabling pagination, disable custom scrollbar
                  if (!isPaginationEnabled) {
          ...
          {/* Custom Scrollbar Toggle (Third Place) */}
           <button
               onClick={toggleCustomScrollbar}
               className={cn(
                   'p-2 rounded-md transition-colors duration-200',
           ...
           >
               {/* Icon for scrollbar - Using a simple line/wave icon */}
               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   {isCustomScrollbarEnabled ? (
                        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /> // X icon when enabled (to disable)
                   ) : (
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /> // Wave/Scroll icon when disabled (to enable)
                   )}
               </svg>
           </button>
...
onClick={() => setShowSortOptions(prev => !prev)}
      ...
      <div className={cn(
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
          ...
          <div className="w-full md:w-auto">
            <button
              ...
              className={cn(
               ...
               (isPaginationEnabled ? filesToDisplay.every(file => selectedFiles.includes(file._id)) && filesToDisplay.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0)
            ...
            >
              {(isPaginationEnabled ? filesToDisplay.every(file => selectedFiles.includes(file._id)) && filesToDisplay.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0) ? 'Deselect All' : 'Select All'}
            </button>
          </div>
          ...
          <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
             {selectedFiles.length > 0 && isPaginationEnabled ? `${selectedFiles.length} of ${sortedFiles.length} total items selected till this page.`
                : `${selectedFiles.length} of ${isPaginationEnabled ? filesToDisplay.length : sortedFiles.length} selected.`
             }
          </div>
    ...
    {/* Custom Scrollbar Track and Dragger (appears above the file list) */}
    {isCustomScrollbarEnabled && !isPaginationEnabled && sortedFiles.length > 0 && (
        <div
            ref={scrollbarTrackRef}
            className={cn(
                "relative w-full h-2.5 rounded-full mb-3 cursor-pointer",
        ...
        >
            <div
                ...
                className={cn(
                    "absolute top-0 h-2.5 rounded-full transition-colors duration-150",
                    isDragging ? (darkMode ? "bg-blue-500" : "bg-blue-700") : (darkMode ? "bg-blue-600 hover:bg-blue-500" : "bg-blue-600 hover:bg-blue-700"),
                    "cursor-grab", // Indicate draggable
                    isDragging && "cursor-grabbing" // Indicate dragging
                )}
                style={{ left: 0, width: '50px' }} // Initial style, will be updated by JS
    ...
    {/* Files Display Area with min-height and horizontal scrolling */}
    <div
       ref={fileListContainerRef}
       className={cn(
          'min-h-[250px] overflow-y-auto', // Always allow vertical scrolling if needed
          // Conditionally enable horizontal scrolling and hide native scrollbar
          isCustomScrollbarEnabled && !isPaginationEnabled ? 'overflow-x-auto custom-scrollbar-hidden' : 'overflow-x-hidden',
          // Apply grid/list layout
          view === 'grid' ? 'grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1 gap-4' // Keep gap for list view too
       )}
    >
      {isLoading ? (
        <div className={cn(
        ...
        <div className={cn(
          'grid gap-4',
          // Ensure grid layout allows horizontal scrolling when custom scrollbar is on
          view === 'grid' ? 'grid-flow-col auto-cols-[minmax(180px,_1fr)] sm:auto-cols-[minmax(200px,_1fr)] md:auto-cols-[minmax(220px,_1fr)] lg:auto-cols-[minmax(240px,_1fr)] xl:auto-cols-[minmax(260px,_1fr)]' : 'grid-cols-1',
          // Add sufficient padding/margin if needed for the last item to be fully visible when scrolled
          isCustomScrollbarEnabled && !isPaginationEnabled ? 'pb-4' : '' // Add padding bottom to avoid scrollbar covering content
        )}>
          {/* Render File Items */}
                  ...
                  <span
                      ...
                      onTouchStart={(e) => {
                          ...
                          e.currentTarget.dataset.pressTimer = setTimeout(() => {
                              handlePageClick();
                          }, 300); // Reduced long press time for better UX
                      }}
                      onTouchEnd={(e) => {
...
<div className="mt-4 text-center">
  <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
    ...
    {/* CSS for hiding native scrollbar */}
    <style jsx>{`
