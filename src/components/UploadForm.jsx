  ...
  return (
    ...
    >
      ...
      {resumableUpload && !isUploading && (
        <div className={`mb-4 p-3 rounded-lg ${darkMode ? 'bg-blue-900/30 border border-blue-700' : 'bg-blue-100 border border-blue-300'}`}>
          <div className="mb-2">
            <h4 className={`font-medium ${darkMode ? 'text-blue-400' : 'text-blue-700'}`}>Resume Previous Upload?</h4>
            <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              Your previous upload of "{getTruncatedFileName(file?.name)}" was interrupted at {resumableUpload.progress}%.
            </p>
          </div>
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium transition-colors"
            >
              Resume
            </button>
            <button
      ...
      {file && !isUploading && !resumableUpload && (
        <div className="flex gap-2">
          <button
            type="submit"
