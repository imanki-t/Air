...
const FileItem = ({ file, refresh, showMetadata }) => {  
  ...
  const [showButtons, setShowButtons] = useState(false); // Hide buttons by default  
  ...
  return (  
    <>  
      {/* File Card */}  
      <div className={`bg-yellow-300 w-full flex flex-col justify-between p-3 sm:p-4 text-sm sm:text-base rounded-xl shadow-lg border-4 border-dashed border-purple-600 overflow-hidden ${showMetadata || showButtons ? 'h-auto' : 'h-[210px]'}`}>  
        {renderPreview()}  
