...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
 ...
 // New state for draggable scroll handle
 const [isScrollHandleEnabled, setIsScrollHandleEnabled] = useState(false);
 ...
 // --- Scroll Handle Drag Logic ---
 useEffect(() => {
    const handleMouseMove = (e) => {
        if (!isDragging || !fileListContainerRef.current || !scrollThumbRef.current) return;

        const container = fileListContainerRef.current;
        const thumb = scrollThumbRef.current;
        const trackHeight = container.offsetHeight; // Height of the visible container (scrollbar track height)
        const scrollHeight = container.scrollHeight; // Total scrollable height of content
        const thumbHeight = thumb.offsetHeight;

        const deltaY = e.clientY - dragStartPosY;
        let newThumbTop = thumbStartTop + deltaY;

        // Clamp thumb position to stay within the track
        ...
        const newScrollTop = scrollPercentage * (scrollHeight - trackHeight);
 ...
 }, [isDragging, dragStartPosY, thumbStartTop]);
 ...
 useEffect(() => {
    const updateThumbPosition = () => {
        const container = fileListContainerRef.current;
        const thumb = scrollThumbRef.current;
        if (!container || !thumb || !isScrollHandleEnabled) return;

        const scrollHeight = container.scrollHeight;
        const scrollTop = container.scrollTop;
        const containerHeight = container.offsetHeight;
        const trackHeight = containerHeight; // In this case, track height is container height

        // Calculate thumb height based on content vs container size
        // Thumb height is proportional to the visible content area
        const thumbHeight = Math.max(20, (containerHeight / scrollHeight) * trackHeight); // Minimum thumb height
        thumb.style.height = `${thumbHeight}px`;

        // Calculate thumb position based on scroll position
        const thumbTop = (scrollTop / (scrollHeight - containerHeight)) * (trackHeight - thumbHeight);
        thumb.style.top = `${thumbTop}px`;
    };

    const container = fileListContainerRef.current;
    if (container && isScrollHandleEnabled) {
        container.addEventListener('scroll', updateThumbPosition);
        // Initial update
        updateThumbPosition();
        // Also update if files change and scrollbar is enabled
        // updateThumbPosition is called inside handleResize, but this covers initial load and file changes when not resizing
 ...
 }, [paginatedFiles, isScrollHandleEnabled]); // Depend on paginatedFiles to update on content change, and isScrollHandleEnabled
 ...
 // Use paginatedFiles only if pagination is enabled AND scroll handle is disabled
 const paginatedFiles = isPaginationEnabled && !isScrollHandleEnabled ? sortedFiles.slice(startIndex, endIndex) : sortedFiles;
 ...
 const toggleSelectAll = () => {
     // Determine the list of files relevant to 'Select All' based on pagination state and scroll handle state
     const filesToConsider = isPaginationEnabled && !isScrollHandleEnabled ?
...
const zipFilename = `KUWUTEN${timestamp}${randomCombo}.zip`;
 ...
 // Handler for the new scroll handle toggle button
 const toggleScrollHandle = () => {
    setIsScrollHandleEnabled(prev => {
        const newState = !prev;
        if (newState) {
            // If enabling scroll handle, disable pagination
            setIsPaginationEnabled(false);
            setCurrentPage(1); // Reset to first page when disabling pagination
        } else {
             // If disabling scroll handle, enable pagination
 ...
 return (
   <div className={cn(
     ...
     <div className="text-center mb-6">
       ...
       <span className={cn('text-sm px-3 py-1 rounded-full inline-block transition-all duration-200', darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600' )}>
       ...
       <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
           ...
           <button
               ...
               className={cn(
                   ...
                   isPaginationEnabled && !isScrollHandleEnabled
                       ? 'bg-blue-600 text-white hover:bg-blue-700'
                       : darkMode ?
 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-600 hover:bg-gray-300',
                    // Disable if scroll handle is enabled
                   isScrollHandleEnabled ? 'opacity-50 cursor-not-allowed' : ''
               )}
 ...
 "Disable Pagination" : "Enable Pagination"}
               disabled={isScrollHandleEnabled}
           >
 ...
 (
                        ...
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0
 11-18 0 9 9 0 0118 0z" /> // Plus icon for enabling
                    )}
                </svg>
           </button>

            {/* Scroll Handle Toggle (Third Icon) */}
            <button
               onClick={toggleScrollHandle}
               className={cn(
                   'p-2 rounded-md transition-colors duration-200',
                   isScrollHandleEnabled
                       ? 'bg-blue-600 text-white hover:bg-blue-700'
                       : darkMode ?
 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'
               )}
               aria-label={isScrollHandleEnabled ?
 "Disable scroll handle" : "Enable scroll handle"}
               aria-pressed={isScrollHandleEnabled}
               title={isScrollHandleEnabled ?
 "Disable Scroll Handle" : "Enable Scroll Handle"}
           >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                     {isScrollHandleEnabled ? (
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /> // Minus icon when on
                    ) : (
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 6h6v12H9V6z" /> // Scrollbar icon
                    )}
                </svg>
           </button>
 ...
 (darkMode ? 'bg-blue-700 text-white' : 'bg-blue-100 text-blue-700 font-medium')
         ...
         <div className={cn('flex items-center rounded-md overflow-hidden', darkMode ? 'bg-gray-700' : 'bg-gray-200')}>
           ...
           <button
             ...
             className={cn('p-2 transition-colors duration-200',
                view === 'grid' ?
 'bg-blue-600 text-white' : darkMode ? 'text-gray-300 hover:bg-gray-600' : 'text-gray-600 hover:bg-gray-300'
             )}
             aria-label="Grid view" aria-pressed={view === 'grid'} title="Grid View"
           >
             <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
               <path strokeLinecap="round" strokeLinejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0
 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
             </svg>
           </button>
         </div>
       </div>
 ...
 'text-blue-300 bg-gray-700 hover:bg-gray-600' : 'text-blue-600 bg-white hover:bg-blue-50'}`
             ...
             >
               {(isPaginationEnabled && !isScrollHandleEnabled ? paginatedFiles.every(file => selectedFiles.includes(file._id)) && paginatedFiles.length > 0 : sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0) ?
 'Deselect All' : 'Select All'}
           ...
           <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
              {selectedFiles.length > 0 && isPaginationEnabled && !isScrollHandleEnabled ?
 `${selectedFiles.length} of ${sortedFiles.length} total items selected till this page.`
                 : `${selectedFiles.length} of ${isPaginationEnabled && !isScrollHandleEnabled ?
 ...
 `Delete ${selectedFiles.length} items` : "Select files to delete"}
     ...
     <div className="min-h-[250px] relative"> {/* Added relative positioning */}
       {/* Custom Scrollbar Track */}
       {isScrollHandleEnabled && (
            <div className={cn(
                'absolute right-0 top-0 h-full w-2 z-10',
                ...
                <div
                    ref={scrollThumbRef}
                    className={cn(
                        'absolute left-0 w-full rounded-full cursor-pointer',
                         darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-blue-500 hover:bg-blue-600'
                    )}
                    onMouseDown={(e) => {
                         setIsDragging(true);
                         setDragStartPosY(e.clientY);
                         setThumbStartTop(scrollThumbRef.current.offsetTop);
                    }}
                     style={{ top: 0, height: '50px' }} // Initial style, will be updated by useEffect
                />
            </div>
       )}

       {/* Files Display Area (Scrollable when scroll handle is enabled) */}
       <div
           ref={fileListContainerRef}
           className={cn(
               'grid gap-4 pr-3', // Added right padding to make space for custom scrollbar
               view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1',
               // Make this div scrollable when scroll handle is enabled
               isScrollHandleEnabled ? 'overflow-y-scroll h-[250px] md:h-[400px]' : '' // Set a fixed height for scrolling
           )}
            style={{ scrollbarWidth: isScrollHandleEnabled ? 'none' : 'auto' }} // Hide native scrollbar when custom one is active
       >
       {isLoading ?
 (
         <>
 ...
 'Try adjusting your search or filter.' : 'Upload some files!'}
     ...
     {/* Pagination Controls at the bottom (Hidden when scroll handle is enabled) */}
     {!isScrollHandleEnabled && isPaginationEnabled && sortedFiles.length > 0 && (
       <div className={cn(
 ...
 'sm:flex-col' : 'sm:flex-row justify-center', // Stack vertically on mobile, row on larger screens
          ...
          {/* Page Indicator/Input (Above buttons on mobile) */}
           <span className={cn(
               "text-sm text-center",
               isMobileView ? 'order-1 mb-2' : 'order-2'
 // Order 1 and margin-bottom on mobile
           )}>
 ...
 'item' : 'items'}? This action cannot be undone.
           ...
           <div className="flex gap-3">
             <button
               ...
               className={cn(
                 'flex-1 px-4
 py-2 rounded-md font-medium transition-colors duration-150 text-sm',
                 darkMode
     ...
     {showBatchShareModal && (
       <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fadeIn backdrop-blur-sm">
         <div
           ...
           className={cn(
             'p-6 rounded-lg
 shadow-xl max-w-md w-full relative border animate-modalIn',
              darkMode ? 'bg-gray-800 border-gray-700 text-gray-200' : 'bg-white border-gray-200 text-gray-800'
            )}
   ...
   <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
     ...
     <style jsx>{`
       ...
       .animate-modalIn { animation: modalIn 0.25s ease-out;
        ...
        /* Hide native scrollbar for the container when custom scrollbar is enabled */
