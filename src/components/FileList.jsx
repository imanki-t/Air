...
[source: 2] import { saveAs } from 'file-saver';
...
[source: 3] const FileItemSkeleton = ({ darkMode }) => (
 <div className={cn(
...
[source: 5] const [filter, setFilter] = useState('all');
 const [view, setView] = useState('list'); // Default to list, adjust based on screen size later
 const [searchInput, setSearchInput] = useState('');
 const [showMetadata, setShowMetadata] = useState(false);
[source: 6] const [sortOption, setSortOption] = useState('default');
 const [showSortOptions, setShowSortOptions] = useState(false);


 // --- Batch Operations ---
 const [selectionMode, setSelectionMode] = useState(false);
[source: 7] const [selectedFiles, setSelectedFiles] = useState([]);
 const [batchOperationLoading, setBatchOperationLoading] = useState(false);
 const [showDeleteConfirmModal, setShowDeleteConfirmModal] = useState(false);
[source: 8] const [batchShareLink, setBatchShareLink] = useState('');
[source: 9] const [showBatchShareModal, setShowBatchShareModal] = useState(false);
...
[source: 10] const [copied, setCopied] = useState(false);
...
[source: 11] // Clear selection when files data changes
   setSelectedFiles([]);
 }, [files]);
...
[source: 9] }, [showBatchShareModal, batchShareLink]);
...
[source: 12] const filtered = files.filter((f) => filter === 'all' || f.metadata?.type === filter);
[source: 13] const visibleFiles = filtered.filter((f) =>
   f.filename.toLowerCase().includes(searchInput.toLowerCase())
 );


[source: 14] const sortedFiles = [...visibleFiles].sort((a, b) => {
   switch (sortOption) {
     case 'size':
     ...
     case 'group': {
[source: 15] const order = { image: 1, video: 2, document: 3, audio: 4 };
 ...
 // --- Handlers ---
[source: 16] const clearSearch = () => setSearchInput('');


 const chooseSortOption = (option) => {
   setSortOption(option);
[source: 17] setShowSortOptions(false);
 };
...
[source: 18] const handleSelectFile = (fileId) => {
...
[source: 24] alert('Some files could not be deleted. Please try again.');
 ...
 const batchDownload = async () => {
   if (selectedFiles.length === 0) return;
[source: 26] setBatchOperationLoading(true);
...
[source: 28] if (!selectedFile) continue;
...
[source: 29] zip.file(selectedFile.filename, response.data);
         completedFiles++;
...
[source: 30] saveAs(zipBlob, `download_${Date.now()}.zip`); // Use a more unique name


   } catch (err) {
     console.error('Batch download failed:', err);
[source: 31] alert('Batch download could not be completed. Some files might have failed.');
   } finally {
     setBatchOperationLoading(false);
[source: 32] // Keep progress modal open slightly longer to show 100%
     setTimeout(() => {
...
[source: 33] }
 ...
 const batchShare = async () => {
   if (selectedFiles.length === 0) return;
[source: 34] setBatchOperationLoading(true);
...
[source: 35] setBatchShareLink(res.data.url || `https://share.example.com/batch/${Date.now()}`);
...
[source: 37] const copyToClipboard = async () => {
   if (!batchShareLink) return;
   try {
     await navigator.clipboard.writeText(batchShareLink);
     setCopied(true);
[source: 38] setTimeout(() => setCopied(false), 2000); // Reset copied state
   } catch (err) {
 ...
 // --- Render Logic ---
[source: 39] const skeletonArray = Array(view === 'grid' ? 8 : 4).fill(0); // Adjust skeleton count based on view
[source: 40, 41] const buttonTextColor = darkMode ? 'text-gray-300' : 'text-gray-600';
...
[source: 43] {visibleFiles.length} item{visibleFiles.length !== 1 ? 's' : ''}
         {filter !== 'all' ? ` (${filter})` : ''}
       </span>
     </div>
...
[source: 49, 50, 51, 52, 53] {/* Selection Mode Toggle (Clipboard Icon) */}
         <button
...
[source: 54, 55, 56, 57, 58, 59] {/* Metadata Toggle */}
         <button
...
[source: 60, 61, 62, 63] {/* Sort Toggle */}
...
[source: 64, 65, 66, 67, 68] {/* List View Toggle */}
         <button
...
[source: 69, 70, 71, 72, 73, 74] {/* Grid View Toggle */}
         <button
...
[source: 91, 92, 93, 94, 95] {['all', 'image', 'video', 'audio', 'document', 'other'].map((type) => (
         <button
           key={type}
...
[source: 98, 99, 100] <div className={cn('text-center py-16 px-4', darkMode ? 'text-gray-400' : 'text-gray-500')}>
...
[source: 101, 102] <div className={cn(
         'grid gap-4',
...
[source: 123, 124, 125, 126, 127, 128, 129] {showDeleteConfirmModal && (
...
[source: 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141] {showBatchShareModal && (
       <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
...
[source: 142, 143, 144, 145, 146, 147] {showBatchDownloadProgress && (
       <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
...
[source: 148] export default FileList;
