...
<div className={cn(
    ...
    viewType === 'grid' ? 'h-[180px] flex-col p-3' : 'h-[80px] items-center p-2', // Adjust height and layout for list view
  ...
  {viewType === 'grid' ? (
    ...
    <div className="flex items-center w-full">
      <div className={cn('w-10 h-10 rounded-full mr-3 shrink-0', darkMode ? 'bg-gray-700' : 'bg-gray-200')}></div>
      <div className="flex-grow space-y-2">
        <div className={cn('h-4 w-5/6 rounded-md', darkMode ? 'bg-gray-700' : 'bg-gray-200')}></div>
...
// Ref for the file items container
const fileItemsContainerRef = useRef(null);
// Ref for the custom scrollbar dragger
const scrollbarDraggerRef = useRef(null);
// State to track if the dragger is being dragged
const [isDragging, setIsDragging] = useState(false);
// State to store the initial mouse/touch position and scroll position when dragging starts
...
// Reset to first page when filter or sort changes, or when pagination is enabled/disabled
useEffect(() => {
  setCurrentPage(1);
}, [filter, sortOption, searchInput, itemsPerPage, isPaginationEnabled]);
...
// Function to update the dragger position based on file list scroll
const updateDraggerPosition = useCallback(() => {
   const container = fileItemsContainerRef.current;
   const dragger = scrollbarDraggerRef.current;
   if (container && dragger && !isPaginationEnabled) {
       ...
       // Calculate the maximum track width the dragger can move in
       const trackWidth = container.parentElement.querySelector('.custom-scrollbar-track')?.clientWidth || 0;
       const maxDraggerLeft = trackWidth - draggerWidth;

       if (maxScrollLeft <= 0 || trackWidth <= 0) {
            dragger.style.left = '0px'; // Reset if no scroll needed or track is zero
            return;
       }

       // Calculate the ratio of current scroll position to max scroll position
       const scrollRatio = scrollLeft / maxScrollLeft;
       // Calculate the dragger's left position based on the ratio and max dragger movement
       const draggerLeft = scrollRatio * maxDraggerLeft;

       dragger.style.left = `${draggerLeft}px`;
   }
}, [isPaginationEnabled]); // Dependency on isPaginationEnabled

// Function to update file list scroll based on dragger position
const updateScrollFromDragger = useCallback((draggerLeft) => {
   const container = fileItemsContainerRef.current;
   const dragger = scrollbarDraggerRef.current;
   if (container && dragger && !isPaginationEnabled) {
       const scrollWidth = container.scrollWidth;
       ...
       if (maxScrollLeft <= 0 || trackWidth <= 0) {
            container.scrollLeft = 0; // Reset if no scroll needed or track is zero
            return;
       }

       // Calculate the ratio of dragger position to max dragger movement
       const draggerRatio = draggerLeft / maxDraggerLeft;
       // Calculate the new scroll position based on the ratio and max scroll position
       const newScrollLeft = draggerRatio * maxScrollLeft;

       container.scrollLeft = newScrollLeft;
   }
}, [isPaginationEnabled]); // Dependency on isPaginationEnabled

// Handle mouse down event on the dragger
const handleMouseDown = useCallback((e) => {
   if (isPaginationEnabled) {
       ...
       // Allow the drag to continue after state update
       // Need a small delay for state update to reflect before starting drag logic
       setTimeout(() => {
            if (fileItemsContainerRef.current) {
                setIsDragging(true);
                dragStartRef.current = {
                    x: e.clientX,
                    scrollLeft: fileItemsContainerRef.current.scrollLeft,
                };
            }
       }, 0); // Use 0 delay to run after current event loop
   } else if (showCustomScrollbar) {
       // If custom scrollbar is already on and pagination is off, start dragging
       setIsDragging(true);
       dragStartRef.current = {
           x: e.clientX,
           scrollLeft: fileItemsContainerRef.current.scrollLeft,
...
}, [isPaginationEnabled, showCustomScrollbar]);

// Handle touch start event on the dragger
...
const handleMouseMove = useCallback((e) => {
   if (!isDragging || isPaginationEnabled) return; // Only drag if isDragging is true and pagination is off
   ...
   if (container && dragger && track) {
       const dx = e.clientX - dragStartRef.current.x; // Change in mouse position
       const trackWidth = track.clientWidth;
       const draggerWidth = dragger.clientWidth;
       const maxDraggerLeft = trackWidth - draggerWidth;

       // Calculate new dragger position (clamped within track bounds)
       let newDraggerLeft = dragger.offsetLeft + dx;
       newDraggerLeft = Math.max(0, Math.min(newDraggerLeft, maxDraggerLeft));

       // Update dragger position visually
       dragger.style.left = `${newDraggerLeft}px`;

       // Update scroll position of the file items container
       const scrollRatio = newDraggerLeft / maxDraggerLeft;
...
}, [isDragging, isPaginationEnabled]);

// Handle touch move event while dragging
const handleTouchMove = useCallback((e) => {
   if (!isDragging || isPaginationEnabled) return;
   e.preventDefault();
...
// Handle mouse up / touch end events to stop dragging
...
// Add and remove global mouse/touch move/up listeners when dragging starts/stops
useEffect(() => {
   if (isDragging) {
...
// Update dragger position when the file list scrolls (e.g., via mouse wheel or trackpad)
useEffect(() => {
   const container = fileItemsContainerRef.current;
   if (container && !isPaginationEnabled) {
       container.addEventListener('scroll', updateDraggerPosition);
       // Initial dragger position update on mount/pagination change
       updateDraggerPosition();
       return () => {
           container.removeEventListener('scroll', updateDraggerPosition);
       };
   }
}, [updateDraggerPosition, isPaginationEnabled]);

// Recalculate dragger position and size when files change or view changes
useEffect(() => {
   updateDraggerPosition();
}, [paginatedFiles, view, updateDraggerPosition]); // Depend on paginatedFiles (or sortedFiles when no pagination) and view

// Calculate dragger width based on content vs container width
const draggerWidth = useCallback(() => {
   const container = fileItemsContainerRef.current;
   if (container && !isPaginationEnabled) {
       const contentWidth = container.scrollWidth;
       const containerWidth = container.clientWidth;
       if (contentWidth <= containerWidth || containerWidth === 0) {
           return 0; // Hide dragger if no scroll needed or container is zero width
       }
       // Dragger width is proportional to the visible area
       const width = (containerWidth / contentWidth) * containerWidth;
       // Ensure minimum dragger width for usability
       return Math.max(20, width); // Minimum width of 20px
   }
   return 0; // Hide dragger if pagination is on or container is not ready
}, [isPaginationEnabled]);
      ...
      <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
          ...
          <button
              onClick={() => {
                  ...
                  // If enabling pagination, hide custom scrollbar
                  if (!isPaginationEnabled) {
          ...
          {/* Toggle Custom Scrollbar (New Button - 3rd position) */}
          <button
              onClick={() => {
                  setShowCustomScrollbar(prev => !prev);
                  // If enabling custom scrollbar, disable pagination
              ...
              aria-label={showCustomScrollbar ? "Hide custom scrollbar" : "Show custom scrollbar"}
              aria-pressed={showCustomScrollbar}
              title={showCustomScrollbar ? "Hide Custom Scrollbar" : "Show Custom Scrollbar"}
          >
               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   {/* Icon for scrollbar - can be customized */}
...
onClick={() => setShowSortOptions(prev => !prev)}
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
          ...
          <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
             {selectedFiles.length > 0 && isPaginationEnabled ? `${selectedFiles.length} of ${sortedFiles.length} total items selected till this page.`
                : `${selectedFiles.length} of ${isPaginationEnabled ? paginatedFiles.length : sortedFiles.length} selected.`
             }
          </div>
    ...
    {/* Added overflow-x-auto and custom-scrollbar styles */}
    <div
       ref={fileItemsContainerRef}
       className={cn(
          'min-h-[250px] relative', // Added relative for absolute positioning of scrollbar
          // Conditional overflow-x based on custom scrollbar and pagination
          showCustomScrollbar && !isPaginationEnabled ? 'overflow-x-auto overflow-y-hidden' : 'overflow-hidden',
          // Hide native scrollbar if custom scrollbar is active
          showCustomScrollbar && !isPaginationEnabled ? 'hide-native-scrollbar' : ''
       )}
    >
      {isLoading ? (
        <div className={cn(
      ...
      ) : (showCustomScrollbar && !isPaginationEnabled ? sortedFiles : paginatedFiles).length > 0 ? (
        <div className={cn(
          'grid gap-4',
          view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1',
           // Ensure items don't wrap when using custom scrollbar
          showCustomScrollbar && !isPaginationEnabled ? 'flex flex-row flex-nowrap' : ''
        )}>
          {/* Render File Items */}
          {(showCustomScrollbar && !isPaginationEnabled ? sortedFiles : paginatedFiles).map(file => (
            <FileItem
              key={file._id}
              file={file}
              darkMode={darkMode}
              ...
              refresh={refresh}
               // Added shrink-0 to prevent items from shrinking in flex row
              className={showCustomScrollbar && !isPaginationEnabled ? 'shrink-0' : ''}
            />
          ))}
        </div>
      ) : (
    ...
    {/* Custom Scrollbar Track and Dragger */}
    {showCustomScrollbar && !isPaginationEnabled && sortedFiles.length > 0 && (
       <div className={cn(
           'custom-scrollbar-track relative w-full h-2 rounded-full mt-4 cursor-pointer',
           darkMode ? 'bg-gray-700' : 'bg-gray-300'
       )}>
           <div
               ref={scrollbarDraggerRef}
               className={cn(
                   'custom-scrollbar-dragger absolute top-0 h-full rounded-full transition-colors duration-200',
...
<div className="mt-4 text-center">
  <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
    ...
    <style jsx>{`
      ...
      .custom-scrollbar-track {
          position: relative; /* Needed for dragger absolute positioning */
      }
