...
const UserNotesDashboard = ({ user, darkMode }) => {
  ...
  return (
    <div
      className={`rounded-2xl p-5 shadow-xl border flex flex-col justify-between transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* Add Note Form */}
        <form onSubmit={addNote} className="flex items-center gap-2 mb-3">
          <input
            type="text"
            placeholder="Write a quick note or pin important text..."
            value={newNoteText}
            onChange={(e) => setNewNoteText(e.target.value)}
            className={`flex-1 px-3 py-2 rounded-xl text-xs sm:text-sm border outline-none transition-all ${
              darkMode
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs transition-all shadow-md disabled:opacity-40 shrink-0"
          >
            Add Note
          </button>
        </form>
        ...
        <div className="space-y-2 max-h-44 overflow-y-auto pr-1 custom-scrollbar">
          {sortedNotes.length === 0 ? (
            <p className={`text-center py-6 text-xs italic ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
              No notes yet. Type a note above and click "Add Note".
            </p>
          ) : (
                ...
                {editingId === note.id ? (
                  <div className="flex-1 flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-2 py-1 bg-green-600 text-white text-[11px] font-bold rounded hover:bg-green-500"
                    >
                      Save
                    </button>
                  </div>
