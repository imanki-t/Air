...
// Re-run if loading state changes
...
// --- Batch Operations ---
  const batchDelete = async () => {
    ...
    if (!token) {
       alert('Authentication required for batch delete.');
...
// Exit selection mode automatically
      ...
      if (err.response && err.response.status === 401) {
          ...
          alert(`Error deleting files: ${err.response.data.error}`);
      } else {
  ...
  const batchDownload = async () => {
    ...
    const toDownload = selectedFiles
      ...
      .filter(Boolean);
// Ensure we only process found files

    const token = getToken(); // Get token for auth
    if (!token) {
       alert('Authentication required for batch download.');
       setBatchOperationLoading(false);
...
// Optional: Add progress state if zipping/uploading takes time
    // const [batchShareProgress, setBatchShareProgress] = useState(0);
// Add this near other state hooks if using progress

    const token = getToken(); // Get token for auth
    if (!token) {
       alert('Authentication required for batch share.');
       setBatchOperationLoading(false);
       setShowBatchShareModal(false); // Close modal on auth error
       return; // Stop if no token
    }
...
// Optional: Update progress state for zipping
      ...
      console.log('Gener ating ZIP blob...'); // Debug log
 ...
 */}
 ...
 justify-center md:justify-end flex-grow md:flex-grow-0">
          {/* Toggle Selection */}
          <button
           ...
           disabled={selectedFiles.length === 0 || batchOperationLoading}
                className={cn(
                  ...
                  selectedFiles.length === 0 || batchOperationLoading
                    ? (darkMode ?
 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-300 text-gray-500 cursor-not-allowed') // Themed disabled
 ...
 (
        ...
        <svg className="mx-auto h-12 w-12 mb-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <p className="text-sm text-gray-400">
             ...
             {searchInput ?
 'Try adjusting your search or filter.' : 'Upload some files to see them here.'}
            </p>
          </div>
        )}
...
// --- Modals ---
 ...
 justify-center z-50 p-4 animate-fadeIn backdrop-blur-sm">
          <div
       ...
       role="alertdialog" aria-modal="true" aria-labelledby="delete-modal-title" aria-describedby="delete-modal-description"
          >
            ...
            <p id="delete-modal-description" className="text-sm mb-6">
              Are you sure you want to permanently delete {selectedFiles.length} selected
 {selectedFiles.length === 1 ? 'item' : 'items'}? This action cannot be undone.
 ...
 QR background transparent
  ...
  value={batchOperationLoading ? 'Generating link...' : batchShareLink || 'Error - No link generated'}
                  ...
                  className={cn(
                    ...
                    'overflow-x-auto', //
 Legislature horizontal scroll if link is very long
