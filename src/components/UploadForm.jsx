...
const UploadForm = ({ refresh, darkMode }) => {
 ...
 const handleCancel = () => {
   ...
   if (pendingResume?.fileId) {
     cleanupIncompleteUpload(pendingResume.fileId); // Clean up incomplete upload on backend
   }
 ...
 return (
   <form
     ...
     className={`mb-6 p-6 rounded-xl shadow-lg transition-colors duration-300 ${
       darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'
     }`}
   >
     <h3 className={`text-2xl font-semibold mb-6 text-center ${darkMode ? 'text-white' : 'text-gray-800'}`}>
       File Uploader
     </h3>
       ...
       <div className={`mb-6 p-5 rounded-lg ${darkMode ? 'bg-blue-800/40 border border-blue-600' : 'bg-blue-50 border border-blue-400'}`}>
         ...
         <div className="flex justify-center gap-4">
           ...
           <button
             ...
             className={`flex-1 px-4 py-2.5 rounded-md font-medium transition-colors shadow-md ${
               darkMode
                 ?
     ...
     {!isUploading && (
       <label
         htmlFor="fileInput"
         className={`block w-full cursor-pointer px-4 py-8 rounded-lg mb-5 text-center transition-colors duration-200 ease-in-out ${
           darkMode
         ...
         <ul className="space-y-2 max-h-60 overflow-y-auto pr-2"> {/* Added max-height and scroll */}
           {files.map((file, index) => (
             <li
               ...
               className={`flex items-center p-3 rounded-md shadow-sm ${darkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-700'}`}
             >
               <div className="flex-grow border-r border-gray-300 pr-4"> {/* Added border-r */}
               ...
               <button
                 ...
                 className={`ml-4 text-red-500 hover:text-red-700 font-bold text-xl leading-none ${darkMode ? 'text-red-400 hover:text-red-600' : ''}`}
                 aria-label={`Remove ${file.name}`}
               >
                 × {/* HTML entity for multiplication sign, often used for 'x' */}
               </button>
             </li>
           ))}
         </ul>
     ...
     {isUploading && uploadFileRef.current && (
       <div className="mb-4 p-4 rounded-lg shadow-inner border border-blue-400">
         <h4 className={`text-lg font-semibold mb-2 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
           Uploading ({currentFileIndex + 1} of {files.length}): {getTruncatedFileName(uploadFileRef.current.name)}
         </h4>
     ...
     {files.length > 0 && !isUploading && (
       <div className="flex gap-3 mt-2">
         <button
           type="submit"
           className={`flex-1 px-4 py-2.5 text-white rounded-md font-medium transition-colors shadow-md ${
         ...
         >
           {files.length === 1 ? 'Clear' : 'Clear All'}
         </button>
       </div>
     )}
       ...
       <div className="mt-6">
         ...
         <ul className="space-y-2 max-h-40 overflow-y-auto pr-2"> {/* Added max-height and scroll */}
           {uploadedFiles.map((file, index) => (
             <li
               key={file.name + file.size + index}
               className={`flex justify-between items-center p-3 rounded-md shadow-sm ${darkMode ? 'bg-green-800/40 text-green-200' : 'bg-green-50 text-green-700'}`}
             >
