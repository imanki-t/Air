...
// Determine the total number of pages based on all visible files
const totalPages = Math.ceil(visible.length / itemsPerPage);
const startIndex = (currentPage - 1) * itemsPerPage;
const endIndex = startIndex + itemsPerPage;

// Determine which set of files to sort based on pagination state
let filesToActuallySort;
if (isPaginationEnabled) {
   // If pagination is enabled, we sort only the files on the current page
   filesToActuallySort = visible.slice(startIndex, endIndex);
} else {
   // If pagination is disabled, we sort all visible files
   filesToActuallySort = [...visible];
}

// Apply sorting to the determined set of files
const displayFiles = [...filesToActuallySort].sort((a, b) => {
 switch (sortOption) {
   case 'name-asc': return a.filename.localeCompare(b.filename);
   case 'name-desc': return b.filename.localeCompare(a.filename);
   case 'size-asc': return (a.length || 0) - (b.length || 0); // Smaller Files
   case 'size-desc': return (b.length || 0) - (a.length || 0); // Larger Files
   case 'date': return new Date(a.uploadDate) - new Date(b.uploadDate); // Oldest First
   case 'default':
...
// The paginatedFiles variable now holds the final list of files to display,
// which are already sliced (if pagination is enabled) and sorted.
...
paginatedFiles : displayFiles; // Use displayFiles for all visible files when pagination is off
...
refresh(); // Refresh file list
   ...
   // For example, a toast notification
...
<path strokeLinecap="round" strokeLinejoin="round" d="M21 16l-4 4m0 0l-4-4m4 4V8" />
         ...
         {showSortOptions && (
           <div
             ...
             className={cn(
               ...
               'max-h-[40vh] sm:max-h-[75vh] overflow-y-auto', // Added max height for scrollability
     ...
     <div className={cn(
       ...
       <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
         ...
         <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
            {selectedFiles.length > 0 && isPaginationEnabled ? `${selectedFiles.length} of ${visible.length} total items selected till this page.`
  ...
  {isPaginationEnabled && visible.length > 0 && ( // Changed from sortedFiles.length to visible.length for totalPages calculation
      <div className={cn(
          'flex items-center justify-center gap-2 sm:gap-3 mt-8', // Increased gap and margin-top
          ...
          <button
              ...
              className={cn(
                  'p-2.5 rounded-lg text-sm font-medium transition-all duration-200 flex items-center justify-center', // Increased padding
          ...
          <span className={cn(
              "text-sm text-center mx-1", // Added margin for spacing
          )}>
              ...
              {isEditingPage ? (
                  <input
                      ...
                      className={cn(
                          'w-16 text-center p-1.5 rounded-md text-sm border', // Slightly increased padding
                  ...
                  <span
                      ...
                      onTouchStart={(e) => {
                          e.currentTarget.dataset.pressTimer = setTimeout(() => {
                              handlePageClick();
                          }, 800); // Adjusted long-press duration
                      }}
                      onTouchEnd={(e) => {
                      ...
                      onMouseLeave={(e) => {
                           // Clear timer if mouse leaves while holding (for mouse users)
                           clearTimeout(e.currentTarget.dataset.pressTimer);
                       }}
                  >
                      {currentPage}
    ...
    {/* Message when pagination is enabled but no files */}
     {isPaginationEnabled && visible.length === 0 && !isLoading && ( // Changed from sortedFiles.length to visible.length
         ...
         <div className="flex flex-col gap-2.5">
            <input
               ...
               className={cn(
                 ...
                 'overflow-x-auto', // Ensure long links are scrollable
...
<div className="mt-4 text-center">
 <p className={cn("text-xs", darkMode ? "text-gray-400" : "text-gray-600")}>
   ...
   <style jsx>{`
     ...
     /* Custom dark mode disabled button style for pagination */
     .dark .bg-gray-750 { background-color: #303742; /* A slightly different shade for disabled */ }
   `}</style>
 </div>
);
};
