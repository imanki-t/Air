...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => {
  ...
  const batchShare = async () => {
    ...
    try {
      // Using the same API endpoint as single file share
      const shareRequests = selectedFiles.map(fileId => {
        return axios.post(`${backendUrl}/api/files/share/${fileId}`);
      });
      
      const responses = await Promise.all(shareRequests);
      const shareUrls = responses.map(res => res.data.url).filter(Boolean);
      
      if (shareUrls.length === 0) {
        throw new Error("Share URLs not received from backend.");
      }
      
      // If there's only one file, use its URL directly
      // Otherwise, still use the batch endpoint (which should be modified on backend to accept array)
      const shareUrl = shareUrls.length === 1 ? shareUrls[0] : `${window.location.origin}/shared-batch/${Date.now()}`;
  ...
  const buttonStyle = darkMode ? 'bg-gray-700 text-gray-300 focus:outline-none' : 'bg-gray-200 text-gray-600 focus:outline-none';
  const activeButtonStyle = 'bg-blue-600 text-white focus:outline-none';
  
  // Action button styles for batch operations
  const batchButtonClasses = {
  ...
  return (
    <div className={`transition-colors duration-300 rounded-lg p-4 shadow-lg w-full mx-auto max-w-7xl ${darkMode ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-800'}`}>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6 items-center justify-between">
        ...
        <div className="relative flex-grow w-full md:w-auto">
          ...
          <input
            ...
            className={`w-full pl-8 pr-10 py-2 rounded-lg transition-colors border ${
              darkMode
      ...
      {/* Selection Mode Bar - Modified for better mobile view */}
      {selectionMode && (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-2 rounded-md text-sm font-medium h-9 ${
                  selectedFiles.length === 0 || batchOperationLoading
                    ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
                    : batchButtonClasses.download
                }`}
              >
                Download
              </button>
