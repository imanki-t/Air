...
const FileList = ({ files = [], refresh, darkMode, isLoading }) => { 
 ...
 // --- Screen Size Detection for Responsive Items Per Page ---
 useEffect(() => { 
   const handleResize = () => { 
     if (window.innerWidth < 768) { // Mobile (adjust breakpoint as needed) 
       setItemsPerPage(6); 
 ...
 'Try adjusting your search or filter.' : 'Upload some files!'} 
     ...
     {/* Pagination Controls at the bottom - UPDATED SECTION */}
     {isPaginationEnabled && sortedFiles.length > 0 && ( 
       <div className={cn( 
         'flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 mt-6', // Changed default justify to between for PC, keeps mobile layout clean
          
         darkMode ? 'text-gray-300' : 'text-gray-700' 
       )}>
         {/* Page Indicator - Centered on mobile, part of flow on PC */}
         <div className="w-full sm:w-auto order-first sm:order-none text-center mb-2 sm:mb-0"> {/* Centered text, bottom margin on mobile */}
           <span className="text-sm">
             ...
             {isEditingPage ? ( 
               <input
                ...
                max={totalPages} 
                 ...
                 onBlur={handlePageInputBlur} 
                  ...
                  className={cn( 
                     'w-16 text-center p-1 rounded-md text-sm border mx-1', // Added margin for spacing
               ...
               <span 
                  ...
                  className={cn( 
                     "font-medium cursor-pointer hover:underline px-1", // Added padding
                     darkMode ? 'text-gray-200 hover:text-white' : 'text-gray-800 hover:text-black' // Hover colors 
                  )}
 ...
 handlePageClick(); 
         ...
         {/* Prev/Next Button Container - Below indicator on mobile, side-by-side on PC */}
         <div className="flex w-full sm:w-auto justify-between items-center gap-2 sm:gap-4 order-last sm:order-none"> {/* Takes full width on mobile */}
           {/* Previous Button */}
           <button
             onClick={handlePreviousPage} 
             disabled={currentPage === 1} 
             className={cn( 
               'flex-1 sm:flex-initial px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200', // flex-1 for mobile equal width
               currentPage === 1
                 ? (darkMode ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-300 text-gray-500 cursor-not-allowed') 
           ...
           >
             {/* Mobile: Prev, PC: Previous Page */}
             <span className="sm:hidden">Prev</span>
             <span className="hidden sm:inline">Previous <span className="hidden md:inline">Page</span></span> 
           </button> 
           ...
           >
                ...
                {/* Mobile: Next, PC: Next Page */} 
             <span className="sm:hidden">Next</span>
             <span className="hidden sm:inline">Next <span className="hidden md:inline">Page</span></span> 
           </button>
         </div>
       </div>
     )}
      {/* Message when pagination is enabled but no files */}
       {isPaginationEnabled && sortedFiles.length === 0 && !isLoading && ( // Added !isLoading check
