...
// --- Scrollbar State ---
const [isScrollbarEnabled, setIsScrollbarEnabled] = useState(false); // New state for custom scrollbar toggle
...
const scrollableContainerRef = useRef(null); // Ref for the scrollable file list container
const scrollThumbRef = useRef(null); // Ref for the custom scrollbar thumb
...
useEffect(() => {
   ...
   if (!scrollableContainer || !scrollThumb || isPaginationEnabled || !isScrollbarEnabled) {
       setIsDragging(false); // Ensure dragging is off if conditions aren't met
       return; // Only enable drag when pagination is off AND scrollbar is enabled
   }
   ...
   const handleMouseMove = (e) => {
       ...
       // Calculate scrollable height and scroll thumb track height
       const trackHeight = scrollableContainer.clientHeight;
       const contentHeight = scrollableContainer.scrollHeight;
       const scrollRatio = trackHeight / contentHeight;
       const thumbPosition = startScrollTop + (dy / scrollRatio);

       scrollableContainer.scrollTop = thumbPosition;
   };
   ...
   const handleThumbMouseDown = (e) => {
       ...
       setIsDragging(true);
       setStartY(e.clientY);
       setStartScrollTop(scrollableContainer.scrollTop);
   };
...
}, [isDragging, startY, startScrollTop, isPaginationEnabled, isScrollbarEnabled]); // Re-run effect if these states change

// Update scrollbar thumb position when container scrolls
useEffect(() => {
   const scrollableContainer = scrollableContainerRef.current;
   const scrollThumb = scrollThumbRef.current;
   ...
   const updateThumbPosition = () => {
       ...
       if (totalHeight <= visibleHeight) {
           scrollThumb.style.height = '0px'; // Hide thumb if not scrollable
           return;
       }
   ...
   scrollableContainer.addEventListener('scroll', updateThumbPosition);
   // Also update position on resize or when content/pagination changes
   ...
   return () => {
       scrollableContainer.removeEventListener('scroll', updateThumbPosition);
        window.removeEventListener('resize', updateThumbPosition);
   };

}, [paginatedFiles.length, isPaginationEnabled, isScrollbarEnabled, sortedFiles.length]); // Depend on content length and scrollbar state
...
11-18 0 9 9 0 0118 0z" /> // Plus icon for enabling
          ...
          {/* Scrollbar Toggle (New Button - 3rd position) */}
          <button
               onClick={() => setIsScrollbarEnabled(prev => !prev)}
               disabled={isPaginationEnabled} // Disable when pagination is ON
               className={cn(
                   'p-2 rounded-md transition-colors duration-200',
                   isPaginationEnabled // Grey out button if pagination is enabled
                       ? (darkMode ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-300 text-gray-500 cursor-not-allowed')
                       : isScrollbarEnabled // Button state when pagination is off
                           ? 'bg-blue-600 text-white hover:bg-blue-700'
                           : (darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-600 hover:bg-gray-300')
               )}
               aria-label={isScrollbarEnabled ? "Hide custom scrollbar" : "Show custom scrollbar"}
               aria-pressed={isScrollbarEnabled}
               title={isPaginationEnabled ? "Enable pagination to use scrollbar" : (isScrollbarEnabled ? "Hide Scrollbar" : "Show Scrollbar")}
           >
              {/* Scrollbar Icon - You can replace this with a different scrollbar icon if preferred */}
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   <path strokeLinecap="round" strokeLinejoin="round" d="M15 15l-2 5L9 3l2 5m5 0h2a9 9 0 017 7v3a2 2 0 01-2 2H7a2 2 0 01-2-2v-3a9 9 0 017-7h2z" />
              </svg>
          </button>
...
`Delete ${selectedFiles.length} items` : "Select files to delete"}
    ...
    <div className={cn(
       "min-h-[250px] relative", // Added relative positioning for scrollbar
       !isPaginationEnabled && isScrollbarEnabled ? 'custom-scrollbar-container' : '' // Apply custom scrollbar class
    )}>
      {/* Files Display Area */}
      {isLoading ?
(
...
(
        <div
           ref={scrollableContainerRef} // Attach ref here
           className={cn(
              'grid gap-4',
              view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1',
              !isPaginationEnabled && isScrollbarEnabled ? 'custom-scrollbar-content' : 'overflow-y-auto' // Apply custom scroll or native
           )}
        >
          {/* Render File Items */}
...
'Try adjusting your search or filter.' : 'Upload some files!'}
       ...
       {!isPaginationEnabled && isScrollbarEnabled && sortedFiles.length > 0 && (
            <div className={cn(
               'custom-scrollbar-track absolute top-0 right-0 w-2 h-full rounded-full',
               darkMode ? 'bg-gray-700' : 'bg-gray-300'
            )}>
                 {/* Custom Scrollbar Thumb */}
                 <div
                     ref={scrollThumbRef} // Attach ref here
                     className={cn(
                         'custom-scrollbar-thumb absolute top-0 left-0 w-full rounded-full cursor-pointer',
                         darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-blue-600 hover:bg-blue-700'
                     )}
                     style={{ height: '40px' }} // Initial arbitrary height, will be calculated by JS
                 ></div>
            </div>
       )}
...
<style jsx>{`
      ...
      .custom-scrollbar-content {
           height: 100%; /* Ensure content fills container for scrolling */
           overflow-y: scroll;
           /* Hide native scrollbar */
           scrollbar-width: none;  /* Firefox */
           -ms-overflow-style: none;  /* Internet Explorer 10+ */
           padding-right: 10px; /* Add padding to prevent content from being hidden by custom scrollbar */
       }
      ...
      .custom-scrollbar-track {
           ...
           background-color: transparent; /* Track background, can be themed */
           right: 4px; /* Position the track inside the container */
           top: 0;
           bottom: 0;
      }
      ...
      .custom-scrollbar-thumb {
          ...
          /* Thumb position will be controlled by JS transform */
          background-color: ${darkMode ? '#3B82F6' : '#1D4ED8'}; /* Default blue theme */
      }

       .custom-scrollbar-thumb:hover {
            background-color: ${darkMode ? '#60A5FA' : '#2563EB'}; /* Lighter blue on hover */
       }
