...
const UploadForm = ({ refresh, darkMode }) => {
  ...
  const saveUploadState = () => {
    ...
    if (uploadFileRef.current) {
      try {
        ...
        reader.readAsDataURL(uploadFileRef.current);
      } catch (err) {
    ...
    if (droppedFile) {
      ...
      if (pendingResume) {
        if (pendingResume.fileName === droppedFile.name && 
          ...
          // Start upload automatically if it's the correct file for resuming
  ...
  return (
    ...
    >
      ...
      {/* Upload area - only show if not currently uploading and not showing pendingResume */}
