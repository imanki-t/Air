...
const [isCustomScrollEnabled, setIsCustomScrollEnabled] = useState(false); // New state for custom scrollbar
...
const scrollbarsRef = useRef(null); // Ref for the Scrollbars component
...
// Use paginatedFiles only if pagination is enabled AND custom scroll is NOT enabled
const paginatedFiles = (isPaginationEnabled && !isCustomScrollEnabled) ? sortedFiles.slice(startIndex, endIndex) : sortedFiles;
...
// --- Page Editing Handlers ---
 const handlePageClick = () => {
    // Only allow editing if pagination is enabled, custom scroll is OFF, and there are pages
...
// --- Custom Scrollbar Toggle Handler ---
const handleCustomScrollToggle = () => {
   setIsCustomScrollEnabled(prev => {
       const newState = !prev;
       if (newState) {
           // If enabling custom scroll, disable pagination
           setIsPaginationEnabled(false);
           // Reset scroll position to top when enabling custom scroll
           if (scrollbarsRef.current) {
               scrollbarsRef.current.scrollTop(0);
           }
       } else {
            // If disabling custom scroll, you might want to re-enable pagination by default,
            // or leave it as it was before. Let's leave it as it was for now,
            // but the user can manually re-enable it.
            // setIsPaginationEnabled(true); // Optional: uncomment to re-enable pagination
       }
       return newState;
   });
};
...
const toggleSelectAll = () => {
    // Determine the list of files relevant to 'Select All' based on pagination & custom scroll state
    // If custom scroll is enabled, we consider ALL sorted files for select all
    const filesToConsider = isCustomScrollEnabled ? sortedFiles : paginatedFiles;
...
// Custom Scrollbar Styling Components
const renderTrack = ({ style, ...props }) => {
  const trackStyle = {
    ...
    bottom: 2,
    top: 2,
    width: 8, // Width of the scrollbar track (the "pipe thing")
    borderRadius: 4,
    backgroundColor: darkMode ? '#4B5563' : '#E5E7EB', // Themed background color
  };
...
const renderThumb = ({ style, ...props }) => {
  const thumbStyle = {
    ...style,
    width: 8, // Width of the scrollbar thumb (the "dragger")
    backgroundColor: darkMode ? '#60A5FA' : '#3B82F6', // Themed thumb color (Blue)
    borderRadius: 4,
  };
      ...
      <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end flex-grow md:flex-grow-0">
          ...
          <button
              onClick={() => setIsPaginationEnabled(prev => {
                  const newState = !prev;
                  if (newState) {
                      // If enabling pagination, disable custom scroll
          ...
          >
           ...
           {/* Custom Scrollbar Toggle (New Button) */}
          <button
              onClick={handleCustomScrollToggle}
              className={cn(
                  'p-2 rounded-md transition-colors duration-200',
              ...
              aria-label={isCustomScrollEnabled ? "Disable custom scrollbar" : "Enable custom scrollbar"}
              aria-pressed={isCustomScrollEnabled}
              title={isCustomScrollEnabled ? "Disable Custom Scrollbar" : "Enable Custom Scrollbar"}
          >
               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   {/* Icon representing a scrollbar, e.g., a narrow rectangle */}
                   <rect x="18" y="4" width="4" height="16" rx="2" ry="2" />
                   <path strokeLinecap="round" strokeLinejoin="round" d="M18 8h4M18 12h4M18 16h4" /> {/* Lines on the scrollbar */}
               </svg>
          </button>
...
onClick={() => setShowSortOptions(prev => !prev)}
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 flex-wrap">
          ...
          <div className="w-full md:w-auto">
            <button
              ...
              className={cn(
               ...
               // Check against sortedFiles if custom scroll is enabled, otherwise paginatedFiles
               (isCustomScrollEnabled ? sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0 : paginatedFiles.every(file => selectedFiles.includes(file._id)) && paginatedFiles.length > 0)
            ...
            >
              {(isCustomScrollEnabled ? sortedFiles.every(file => selectedFiles.includes(file._id)) && sortedFiles.length > 0 : paginatedFiles.every(file => selectedFiles.includes(file._id)) && paginatedFiles.length > 0) ? 'Deselect All' : 'Select All'}
            </button>
          </div>
          ...
          <div className={cn('text-sm flex-grow text-center md:text-left order-last md:order-none w-full md:w-auto pt-2 md:pt-0', darkMode ? 'text-gray-400' : 'text-gray-600')}>
             {selectedFiles.length > 0 && isPaginationEnabled && !isCustomScrollEnabled ? `${selectedFiles.length} of ${sortedFiles.length} total items selected till this page.`
                : `${selectedFiles.length} of ${isCustomScrollEnabled ? sortedFiles.length : paginatedFiles.length} selected.`
             }
          </div>
    ...
    {/* Files Display Area with min-height and conditional scrolling */}
    {/* Apply max-height and overflow-y-auto to the container when custom scroll is enabled */}
    {isCustomScrollEnabled ? (
        <Scrollbars
            ref={scrollbarsRef} // Attach ref
            style={{ width: '100%', height: 'calc(100vh - 300px)' }} // Example height, adjust as needed
            renderTrackVertical={renderTrack} // Use custom track renderer
            renderThumbVertical={renderThumb} // Use custom thumb renderer
            autoHide // Hide scrollbar when not needed
            autoHideTimeout={1000}
            autoHideDuration={200}
        >
           <div className={cn(
             'grid gap-4 pr-2', // Add padding-right to prevent content from being hidden by scrollbar
             view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1'
           )}>
             {isLoading ? (
                // Render Skeletons within the scrollable area
                Array(view === 'grid' ? 10 : 5).fill().map((_, i) => (
                  <FileItemSkeleton key={`skel-${i}`} darkMode={darkMode} />
                ))
             ) : paginatedFiles.length > 0 ? ( // paginatedFiles is actually all sortedFiles when custom scroll is on
                // Render File Items within the scrollable area
        ...
        // Render original div with pagination when custom scroll is disabled
        <div className={cn(
          'min-h-[250px] grid gap-4',
          view === 'grid' ? 'grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1'
        )}>
          {isLoading ? (
            // Render Skeletons
    ...
    {/* Only show pagination controls if pagination is enabled AND custom scroll is disabled */}
